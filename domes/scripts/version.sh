#!/bin/bash
# =============================================================================
# AgentOS domes Module Version Management Script
# Version: 1.0.0
...
get_git_version() {
    local tag=""
    if git describe --tags --exact-match 2>/dev/null | grep -q "^domes-v"; then
...
check_version_consistency() {
    ...
    if [[ -f "${MODULE_DIR}/include/domes.h" ]]; then
        header_version=$(grep -oP 'DOMES_VERSION\s+"?\K[0-9]+\.[0-9]+\.[0-9]+' "${MODULE_DIR}/include/domes.h" | head -1)
    fi
    
    echo "VERSION file: $file_version"
    echo "CMakeLists.txt: $cmake_version"
    echo "domes.h: $header_version"
    ...
    if [[ -n "$header_version" ]] && [[ "$file_version" != "$header_version" ]]; then
        echo "WARNING: Version mismatch between VERSION file and domes.h" >&2
    fi
}
...
generate_changelog_entry() {
    ...
    if [[ -n "$previous_version" ]]; then
        commit_range="domes-v${previous_version}..HEAD"
    else
    ...
    local changes=$(git log $commit_range --oneline --no-decorate -- domes/ 2>/dev/null | head -20)
...
generate_artifact_name() {
    ...
    echo "agentos-domes-${version}-${os}-${arch}"
}
...
print_usage() {
    cat << EOF
AgentOS domes Module Version Management
