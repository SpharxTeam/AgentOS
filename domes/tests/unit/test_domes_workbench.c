...
static int test_workbench_default_config(void) {
    ...
    assert(config.redirect_stderr == true);
...
static int test_workbench_config_with_limits(void) {
    ...
    config.limits.max_memory_bytes = 256 * 1024 * 1024;
    config.limits.max_cpu_time_ms = 60000;
    config.limits.max_processes = 10;
    config.limits.max_threads = 20;
...
static int test_container_config_init(void) {
    ...
    assert(config.runtime == CONTAINER_RUNTIME_AUTO);
    assert(strcmp(config.resources.network_mode, "none") == 0);
    assert(config.resources.readonly_rootfs == true);
    assert(config.resources.memory_limit == 512 * 1024 * 1024);
...
int main(int argc, char* argv[]) {
    ...
    printf("========================================\n");
    printf("domes Workbench Unit Tests\n");
    printf("========================================\n\n");
