...
`workbench/` 是 Domes 安全沙箱的**隔离执行环境**，为 Agent 提供安全的运行沙箱。
...
```c
// 硬限制和软限制
config.memory_hard_limit_mb = 512;   // 硬限制，超出即 OOM
config.memory_soft_limit_mb = 400;   // 软限制，触发告警

// OOM 策略
config.oom_policy = WORKBENCH_OOM_KILL;  // 直接杀死
// 或
config.oom_policy = WORKBENCH_OOM_PAUSE; // 暂停并告警
```
...
```c
// CPU 配额（微秒级别）
config.cpu_quota_us = 500000;   // 每周期 500ms
config.cpu_period_us = 1000000; // 周期 1s
// 相当于 50% CPU

// CPU 核心绑定
config.cpu_affinity = {0, 1};   // 只允许在 Core 0,1 运行
```

#### 进程数限制

```c
config.pids_max = 10;  // 包括子进程
```
...
```c
// 创建私有挂载点
config.root_dir = "/var/agentos/sandbox/root";
config.mount_tmpfs = true;
config.tmpfs_size_mb = 64;
...
int main() {
    ...
    // 2. 创建沙箱环境
    workbench_t* sandbox = domes_workbench_create(domes,
    ...
    if (pid < 0) {
        ...
        domes_workbench_destroy(domes, sandbox);
        return -1;
    }
    ...
    // 6. 记录审计日志
    if (WIFEXITED(status)) {
        domes_audit(domes, AUDIT_LEVEL_INFO,
...
if (trusted_agent) {
    // 受信任的 Agent：进程模式即可
    config.isolation_type = WORKBENCH_ISOLATION_PROCESS;
} else if (untrusted_agent) {
    // 不受信任：容器模式
    config.isolation_type = WORKBENCH_ISOLATION_CONTAINER;
} else if (high_security_required) {
    // 高安全场景：考虑 gVisor 或 Kata Containers
    config.isolation_type = WORKBENCH_ISOLATION_GVISOR;
}
```
...
if (cpu_intensive_agent) {
    config.cpu_quota_percent = 200;  // 2 核
    config.memory_limit_mb = 2048;   // 2GB
} else if (io_intensive_agent) {
    config.io_read_bps = 100 * 1024 * 1024;  // 100MB/s
    config.io_write_bps = 50 * 1024 * 1024;  // 50MB/s
} else {
    // 通用配置
...
```c
// ✅ 推荐：禁用不必要的功能
config.network_enabled = false;      // 默认禁用网络
config.device_access = false;        // 禁用设备访问
config.capabilities = 0;             // 清空所有能力

// 按需开放
if (needs_network) {
    config.allowed_ports = (int[]){80, 443, -1};  // 仅允许特定端口
}
```
...
while (workbench_is_running(sandbox)) {
    ...
    // 内存接近限制告警
    if (stats.memory_usage_mb > config.memory_limit_mb * 0.9) {
        LOG_WARNING("Memory near limit: %.2fMB / %dMB",
            stats.memory_usage_mb, config.memory_limit_mb);
    }
...
- [容器安全指南](../../../partdocs/specifications/container_security.md)
