...
int domes_vault_init(const domes_vault_config_t* config) {
    ...
    if (config) {
        memcpy(&g_vault_ctx.default_config, config, sizeof(domes_vault_config_t));
    } else {
...
int domes_vault_open(const char* vault_id, const char* password, domes_vault_t** vault) {
    ...
    memcpy(&v->config, &g_vault_ctx.default_config, sizeof(domes_vault_config_t));
