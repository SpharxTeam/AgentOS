/**
 * @file domes_runtime_protection.c
 * @brief 增强运行时保护 - seccomp、CFI 等多层防护实现
 * @author Spharx
 * @date 2026
 */
...
#define DOMES_MAX_SECCOMP_RULES 256
#define DOMES_MAX_CFI_TARGETS 4096
#define DOMES_MAX_VIOLATION_HISTORY 128
...
typedef struct {
    domes_violation_event_t events[DOMES_MAX_VIOLATION_HISTORY];
    size_t count;
    size_t head;
} violation_history_t;
...
static struct {
    int initialized;
    domes_runtime_protect_config_t config;
    domes_protection_status_t status;
    
    seccomp_rule_internal_t seccomp_rules[DOMES_MAX_SECCOMP_RULES];
    size_t seccomp_rule_count;
    uint64_t seccomp_allowed_count;
    uint64_t seccomp_denied_count;
    
    cfi_target_t cfi_targets[DOMES_MAX_CFI_TARGETS];
    size_t cfi_target_count;
    uint64_t cfi_check_count;
    uint64_t cfi_violation_count;
    
    violation_history_t violations;
    domes_protection_stats_t stats;
    
    uint8_t code_hash[32];
    uint8_t data_hash[32];
    int hashes_computed;
    
    void (*violation_callback)(const domes_violation_event_t* event);
...
static uint32_t domes_get_pid(void) {
#ifdef _WIN32
...
static uint32_t domes_get_tid(void) {
#ifdef _WIN32
...
static void domes_record_violation(domes_violation_type_t type, const char* details, const char* syscall_name) {
    if (g_runtime_protect.violations.count >= DOMES_MAX_VIOLATION_HISTORY) {
        g_runtime_protect.violations.head = (g_runtime_protect.violations.head + 1) % DOMES_MAX_VIOLATION_HISTORY;
    }
    
    size_t idx = (g_runtime_protect.violations.head + g_runtime_protect.violations.count) % DOMES_MAX_VIOLATION_HISTORY;
    if (g_runtime_protect.violations.count < DOMES_MAX_VIOLATION_HISTORY) {
        g_runtime_protect.violations.count++;
    }
    
    domes_violation_event_t* event = &g_runtime_protect.violations.events[idx];
    event->type = type;
    event->timestamp = domes_get_time_ms();
    event->pid = domes_get_pid();
    event->tid = domes_get_tid();
    free(event->details);
    event->details = details ? domes_strdup(details) : NULL;
    free(event->syscall_name);
    event->syscall_name = syscall_name ? domes_strdup(syscall_name) : NULL;
...
int domes_runtime_protect_init(const domes_runtime_protect_config_t* config) {
    ...
    if (config) {
        g_runtime_protect.config = *config;
    } else {
        g_runtime_protect.config.level = DOMES_PROTECT_BASIC;
        g_runtime_protect.config.memory.enable_aslr = true;
        g_runtime_protect.config.memory.enable_dep = true;
        g_runtime_protect.config.seccomp.default_action = 0;
        g_runtime_protect.config.integrity.check_interval_ms = 60000;
        g_runtime_protect.config.enable_audit = true;
    }
    
    g_runtime_protect.status = DOMES_PROTECT_STATUS_INACTIVE;
...
void domes_runtime_protect_cleanup(void) {
    ...
    for (size_t i = 0; i < DOMES_MAX_VIOLATION_HISTORY; i++) {
...
int domes_runtime_protect_enable(const domes_runtime_protect_config_t* config) {
    if (!g_runtime_protect.initialized) {
        int result = domes_runtime_protect_init(config);
        if (result != 0) return result;
    } else if (config) {
        g_runtime_protect.config = *config;
    }
    
    if (g_runtime_protect.config.memory.enable_dep ||
        g_runtime_protect.config.memory.enable_aslr) {
        int result = domes_memory_protect_enable(&g_runtime_protect.config.memory);
        if (result != 0) return result;
    }
    
    if (g_runtime_protect.config.cfi.enable_cfi) {
        int result = domes_cfi_enable(&g_runtime_protect.config.cfi);
        if (result != 0) return result;
    }
    
    if (g_runtime_protect.config.seccomp.enable_seccomp) {
        int result = domes_seccomp_enable(&g_runtime_protect.config.seccomp);
        if (result != 0) return result;
    }
    
    if (g_runtime_protect.config.integrity.enable_code_integrity ||
        g_runtime_protect.config.integrity.enable_data_integrity) {
        int result = domes_integrity_enable(&g_runtime_protect.config.integrity);
        if (result != 0) return result;
    }
    
    g_runtime_protect.status = DOMES_PROTECT_STATUS_ACTIVE;
    return 0;
}
...
int domes_runtime_protect_get_config(domes_runtime_protect_config_t* config) {
    if (!config) return -1;
    *config = g_runtime_protect.config;
    return 0;
}

int domes_memory_protect_enable(const domes_memory_protect_config_t* config) {
    if (!config) return -1;
...
int domes_cfi_enable(const domes_cfi_config_t* config) {
    if (!config) return -1;
...
int domes_cfi_register_target(void* source, void* target) {
    ...
    if (g_runtime_protect.cfi_target_count >= DOMES_MAX_CFI_TARGETS) return -1;
...
int domes_cfi_verify_transfer(void* source, void* target) {
    ...
    domes_record_violation(DOMES_VIOLATION_CONTROL_FLOW, "Invalid control flow transfer", NULL);
...
int domes_seccomp_enable(const domes_seccomp_config_t* config) {
    if (!config) return -1;
    ...
    if (config->allowed_syscalls) {
        for (size_t i = 0; i < config->syscall_count && 
             g_runtime_protect.seccomp_rule_count < DOMES_MAX_SECCOMP_RULES; i++) {
            seccomp_rule_internal_t* rule = &g_runtime_protect.seccomp_rules[g_runtime_protect.seccomp_rule_count];
            rule->syscall_name = domes_strdup(config->allowed_syscalls[i]);
            rule->action = 0;
            rule->arg_index = 0;
            rule->arg_value = 0;
            rule->op[0] = '\0';
...
int domes_seccomp_allow(const char* syscall_name) {
    ...
    if (g_runtime_protect.seccomp_rule_count >= DOMES_MAX_SECCOMP_RULES) return -1;
    
    seccomp_rule_internal_t* rule = &g_runtime_protect.seccomp_rules[g_runtime_protect.seccomp_rule_count];
    rule->syscall_name = domes_strdup(syscall_name);
    rule->action = 0;
    rule->arg_index = 0;
    rule->arg_value = 0;
    rule->op[0] = '\0';
...
int domes_seccomp_add_rule(const char* syscall_name, uint32_t arg_index, const char* op,
...
int domes_seccomp_check(const char* syscall_name) {
    ...
    for (size_t i = 0; i < g_runtime_protect.seccomp_rule_count; i++) {
        ...
        if (strcmp(rule->syscall_name, syscall_name) == 0) {
            if (rule->action == 0) {
                ...
                domes_record_violation(DOMES_VIOLATION_SYSCALL, "Blocked syscall", syscall_name);
                return 0;
            }
        }
    }
    
    if (g_runtime_protect.config.seccomp.default_action == 0) {
        g_runtime_protect.seccomp_allowed_count++;
        return 1;
    }
    ...
    g_runtime_protect.seccomp_denied_count++;
    g_runtime_protect.stats.syscall_denied++;
    domes_record_violation(DOMES_VIOLATION_SYSCALL, "Default deny", syscall_name);
    return 0;
}
...
int domes_integrity_enable(const domes_integrity_config_t* config) {
    if (!config) return -1;
    ...
    if (config->enable_code_integrity) {
        domes_integrity_compute_code_hash(g_runtime_protect.code_hash);
    }
...
int domes_integrity_check(void) {
    ...
    if (domes_integrity_compute_code_hash(current_hash) != 0) {
        g_runtime_protect.stats.integrity_failures++;
        domes_record_violation(DOMES_VIOLATION_INTEGRITY, "Failed to compute code hash", NULL);
        return -1;
    }
    
    if (memcmp(current_hash, g_runtime_protect.code_hash, 32) != 0) {
        g_runtime_protect.stats.integrity_failures++;
        g_runtime_protect.status = DOMES_PROTECT_STATUS_COMPROMISED;
        domes_record_violation(DOMES_VIOLATION_INTEGRITY, "Code integrity violation detected", NULL);
...
int domes_integrity_verify_code(const uint8_t* expected_hash) {
    ...
    uint8_t current_hash[32];
    if (domes_integrity_compute_code_hash(current_hash) != 0) return -1;
...
int domes_violation_get_last(domes_violation_event_t* event) {
    ...
    size_t idx = (g_runtime_protect.violations.head + g_runtime_protect.violations.count - 1) % DOMES_MAX_VIOLATION_HISTORY;
...
const char* domes_protection_level_string(domes_protection_level_t level) {
    switch (level) {
        case DOMES_PROTECT_NONE: return "None";
        case DOMES_PROTECT_BASIC: return "Basic";
        case DOMES_PROTECT_ENHANCED: return "Enhanced";
        case DOMES_PROTECT_MAXIMUM: return "Maximum";
...
const char* domes_protection_status_string(domes_protection_status_t status) {
    switch (status) {
        case DOMES_PROTECT_STATUS_INACTIVE: return "Inactive";
        case DOMES_PROTECT_STATUS_ACTIVE: return "Active";
        case DOMES_PROTECT_STATUS_VIOLATION: return "Violation";
        case DOMES_PROTECT_STATUS_COMPROMISED: return "Compromised";
        default: return "Unknown";
    }
}
...
const char* domes_violation_type_string(domes_violation_type_t type) {
    switch (type) {
        case DOMES_VIOLATION_NONE: return "None";
        case DOMES_VIOLATION_SYSCALL: return "Syscall Violation";
        case DOMES_VIOLATION_MEMORY: return "Memory Violation";
        case DOMES_VIOLATION_CONTROL_FLOW: return "Control Flow Violation";
        case DOMES_VIOLATION_INTEGRITY: return "Integrity Violation";
        case DOMES_VIOLATION_RESOURCE: return "Resource Violation";
        default: return "Unknown";
    }
}
...
int domes_protection_get_capabilities(char*** capabilities, size_t* count) {
    ...
    *capabilities = (char**)malloc(*count * sizeof(char*));
    ...
    for (size_t i = 0; i < *count; i++) {
        (*capabilities)[i] = domes_strdup(caps[i]);
    }
