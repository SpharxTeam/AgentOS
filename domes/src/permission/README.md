# Domes 权限裁决模块 (Permission Engine)
...
`permission/` 是 Domes 安全沙箱的**访问控制核心**，基于规则引擎实现细粒度的权限管理。
...
permission_engine_t* engine = permission_engine_create(
    "./config/permission_rules.yaml"  // 规则文件路径
);
...
int main() {
    // 1. 初始化 domes（包含 permission）
    domes_config_t config = {
        .permission_enabled = true,
        .permission_rules_path = "./config/permission_rules.yaml",
    ...
    domes_permission_add_rule(domes, &new_rule);
...
- [安全策略配置](../../../config/security/README.md)
