...
domes_monitoring_t* domes_monitoring_create(const monitoring_config_t* config) {
    ...
    if (config) {
        memcpy(&mgr->config, config, sizeof(monitoring_config_t));
    } else {
        memset(&mgr->config, 0, sizeof(monitoring_config_t));
        mgr->config.backend = MONITORING_BACKEND_PROMETHEUS;
        mgr->config.prometheus.listen_addr = "0.0.0.0";
        mgr->config.prometheus.port = 9090;
        mgr->config.prometheus.endpoint = "/metrics";
        mgr->config.reporting_interval_ms = 10000;
    }
...
const char* domes_monitoring_get_listen_addr(domes_monitoring_t* mgr) {
    ...
    snprintf(addr, sizeof(addr), "%s:%u",
            mgr->config.prometheus.listen_addr,
            mgr->config.prometheus.port);
    domes_rwlock_unlock(&mgr->lock);
...
monitoring_config_t* monitoring_config_create_prometheus(uint16_t port) {
    monitoring_config_t* config = (monitoring_config_t*)domes_mem_alloc(sizeof(monitoring_config_t));
    if (!config) return NULL;

    memset(config, 0, sizeof(monitoring_config_t));

    config->backend = MONITORING_BACKEND_PROMETHEUS;
    config->prometheus.listen_addr = "0.0.0.0";
    config->prometheus.port = port;
    config->prometheus.endpoint = "/metrics";
    config->reporting_interval_ms = 10000;
    config->buffer_size = MAX_METRICS_BUFFER;
...
int domes_monitoring_init_instance(const monitoring_config_t* config) {
    ...
    g_monitoring = domes_monitoring_create(config);
    if (!g_monitoring) {
        domes_rwlock_unlock(&g_monitoring_lock);
        return -1;
    }
