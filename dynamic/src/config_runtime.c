...
agentos_error_t config_manager_update_value(config_manager_t* mgr, const char* key, const char* value) {
    ...
    if (strcmp(key, "server.host") == 0 && cJSON_IsString(json_value)) {
        ...
        mgr->config.server.host = new_host;
        ...
        if (strcmp(json_value->valuestring, "DEBUG") == 0) mgr->config.log.log_level = LOG_LEVEL_DEBUG;
        else if (strcmp(json_value->valuestring, "INFO") == 0) mgr->config.log.log_level = LOG_LEVEL_INFO;
        else if (strcmp(json_value->valuestring, "WARN") == 0) mgr->config.log.log_level = LOG_LEVEL_WARN;
        else if (strcmp(json_value->valuestring, "ERROR") == 0) mgr->config.log.log_level = LOG_LEVEL_ERROR;
