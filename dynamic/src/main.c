...
/* 全局服务器实例（用于信号处理） */
static dynamic_server_t* g_server = NULL;

/* 命令行参数解析 */
static void print_usage(const char* prog) {
    printf("Usage: %s [OPTIONS]\n\n", prog);
    printf("AgentOS Dynamic Runtime - Gateway Layer\n\n");
    printf("Options:\n");
...
static void print_version(void) {
    printf("AgentOS Dynamic Runtime v%s (ABI %u)\n",
        dynamic_get_version(),
        dynamic_get_abi_version());
...
int main(int argc, char** argv) {
    ...
    server_set_default_config(&config);
    ...
    while ((opt = getopt_long(argc, argv, "h:p:w:W:sm:t:v", 
        ...
        switch (opt) {
            case 'h':
                config.http_host = optarg;
                break;
            case 'p':
                config.http_port = (uint16_t)atoi(optarg);
                break;
            case 'w':
                config.ws_host = optarg;
                break;
            case 'W':
                config.ws_port = (uint16_t)atoi(optarg);
                break;
            case 's':
                config.enable_stdio = 0;
                break;
            case 'm':
                config.max_sessions = (uint32_t)atoi(optarg);
                break;
            case 't':
                config.session_timeout_sec = (uint32_t)atoi(optarg);
                break;
            case 'v':
                print_version();
                return 0;
    ...
    AGENTOS_LOG_INFO("AgentOS Dynamic Runtime v%s starting...", 
        dynamic_get_version());
    ...
    /* 创建服务器实例 */
    agentos_error_t err = dynamic_server_create(&config, &g_server);
    ...
    AGENTOS_LOG_INFO("  HTTP:      %s:%u", config.http_host, config.http_port);
    AGENTOS_LOG_INFO("  WebSocket: %s:%u", config.ws_host, config.ws_port);
    AGENTOS_LOG_INFO("  Stdio:     %s", config.enable_stdio ? "enabled" : "disabled");
    AGENTOS_LOG_INFO("  Sessions:  max=%u, timeout=%us", 
        config.max_sessions, config.session_timeout_sec);
    
    /* 启动服务器（阻塞） */
    err = dynamic_server_start(g_server);
    if (err != AGENTOS_SUCCESS) {
        AGENTOS_LOG_ERROR("Server error: %s", agentos_strerror(err));
        dynamic_server_destroy(g_server);
        return 1;
    }
