...
ratelimiter_t* ratelimiter_create(const ratelimit_config_t* config) {
    ...
    if (config) {
        ...
        /* 默认配置 */
        limiter->config.requests_per_second = 100;
        limiter->config.burst_size = 20;
        limiter->config.window_ms = 1000;
...
ratelimit_result_t ratelimiter_check(ratelimiter_t* limiter, const char* client_id) {
    ...
    uint64_t window_ns = (uint64_t)limiter->config.window_ms * 1000000ULL;
    ...
    uint32_t limit = limiter->config.requests_per_second + limiter->config.burst_size;
...
agentos_error_t ratelimiter_get_stats(
    ...
    int len = asprintf(&json,
        ...
        "\"total_denied\":%llu"
        "}",
        limiter->config.enabled ? "true" : "false",
        limiter->config.requests_per_second,
        limiter->config.burst_size,
        limiter->config.window_ms,
