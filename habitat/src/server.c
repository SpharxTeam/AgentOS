...
// 全局实例（供信号处理）
struct habitat_server* g_server = NULL;
...
static void set_default_config(habitat_config_t* cfg) {
...
int habitat_start(const habitat_config_t* user_cfg) {
    if (g_server) {
        AGENTOS_LOG_ERROR("Habitat already running");
        return -1;
    }
    struct habitat_server* server = (struct habitat_server*)calloc(1, sizeof(struct habitat_server));
    if (!server) return -1;
