...
lodges_error_t lodges_memory_init(void) {
    ...
    ensure_directory("lodges/kernel/memory/pools");
    ensure_directory("lodges/kernel/memory/stats");
    ensure_directory("lodges/kernel/memory/allocations");
...
lodges_error_t lodges_memory_record_pool(const lodges_memory_pool_t* pool) {
    ...
    memcpy(&node->pool, pool, sizeof(lodges_memory_pool_t));
...
lodges_error_t lodges_memory_get_pool(const char* pool_id, lodges_memory_pool_t* pool) {
    if (!pool_id || !pool) {
        return lodges_ERR_INVALID_PARAM;
    }
    ...
    while (node) {
        if (strcmp(node->pool.pool_id, pool_id) == 0) {
            memcpy(pool, &node->pool, sizeof(lodges_memory_pool_t));
            pthread_mutex_unlock(&g_memory_data.lock);
            return lodges_SUCCESS;
        }
        node = node->next;
...
lodges_error_t lodges_memory_update_pool_usage(const char* pool_id, size_t used_size, uint32_t free_block_count) {
    if (!pool_id) {
        return lodges_ERR_INVALID_PARAM;
    }
...
lodges_error_t lodges_memory_record_allocation(const lodges_memory_allocation_t* allocation) {
    ...
    memcpy(&node->allocation, allocation, sizeof(lodges_memory_allocation_t));
...
lodges_error_t lodges_memory_get_allocation(const char* allocation_id, lodges_memory_allocation_t* allocation) {
    ...
    while (node) {
        if (strcmp(node->allocation.allocation_id, allocation_id) == 0) {
            memcpy(allocation, &node->allocation, sizeof(lodges_memory_allocation_t));
            pthread_mutex_unlock(&g_memory_data.lock);
            return lodges_SUCCESS;
        }
        node = node->next;
...
lodges_error_t lodges_memory_free_allocation(const char* allocation_id) {
    if (!allocation_id) {
        return lodges_ERR_INVALID_PARAM;
    }
...
lodges_error_t lodges_memory_get_stats(uint32_t* pool_count, uint32_t* total_allocations, uint64_t* total_size) {
    if (!g_memory_data.initialized) {
        return lodges_ERR_NOT_INITIALIZED;
    }
