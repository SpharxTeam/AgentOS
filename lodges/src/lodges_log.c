...
#define lodges_LOG_MAX_LINE_LEN 4096
#define lodges_LOG_BUFFER_SIZE 8192
#define lodges_LOG_MAX_SERVICE_LEN 64

static lodges_log_level_t g_log_level = lodges_LOG_INFO;
...
typedef struct {
    char service_name[lodges_LOG_MAX_SERVICE_LEN];
    FILE* file;
 ...
 */
static const char* level_to_string(lodges_log_level_t level) {
    switch (level) {
        case lodges_LOG_ERROR: return "ERROR";
        case lodges_LOG_WARN: return "WARN";
        case lodges_LOG_INFO: return "INFO";
        case lodges_LOG_DEBUG: return "DEBUG";
...
static const char* get_log_base_path(void) {
    static char base_path[256] = "lodges/logs";
    return base_path;
}
...
static FILE* get_service_log_file(const char* service) {
    ...
    if (g_service_log_count < 32) {
        ...
        if (fp) {
            strncpy(g_service_logs[g_service_log_count].service_name, service, lodges_LOG_MAX_SERVICE_LEN - 1);
...
lodges_error_t lodges_log_init(void) {
    ...
    ensure_directory("lodges/logs/kernel");
    ensure_directory("lodges/logs/services");
    ensure_directory("lodges/logs/apps");

    update_current_date();
    g_main_log_file = fopen("lodges/logs/kernel/agentos.log", "a");
    if (!g_main_log_file) {
        return lodges_ERR_FILE_OPEN_FAILED;
    }

    g_initialized = true;
    g_log_level = lodges_LOG_INFO;
...
void lodges_log_shutdown(void) {
    if (!g_initialized) {
        fprintf(stderr, "[lodges_LOG WARN] Shutdown called but not initialized\n");
        return;
    }
    ...
    if (g_main_log_file) {
        ...
        fprintf(stdout, "[lodges_LOG INFO] Main log file closed\n");
    ...
    fprintf(stdout, "[lodges_LOG INFO] Closed %zu service log files\n", g_service_log_count);
    g_service_log_count = 0;
    pthread_mutex_unlock(&g_service_lock);

    g_initialized = false;
    fprintf(stdout, "[lodges_LOG INFO] Logging system shutdown complete\n");

    pthread_mutex_unlock(&g_log_lock);
}

void lodges_log_write(lodges_log_level_t level, 
                        const char* service, 
                        const char* trace_id, 
                        const char* file, 
                        int line, 
    ...
    lodges_log_writev(level, service, trace_id, file, line, format, args);
    va_end(args);
}

void lodges_log_writev(lodges_log_level_t level, 
                         const char* service, 
                         const char* trace_id, 
                         const char* file, 
                         int line, 
    ...
    char message[lodges_LOG_MAX_LINE_LEN];
...
lodges_log_level_t lodges_log_get_level(void) {
    return g_log_level;
}

void lodges_log_set_level(lodges_log_level_t level) {
    g_log_level = level;
}

lodges_error_t lodges_log_get_service_path(const char* service, char* buffer, size_t buffer_size) {
    if (!buffer || buffer_size == 0) {
        fprintf(stderr, "[lodges_LOG ERROR] Invalid buffer parameter\n");
...
lodges_error_t lodges_log_rotate(void) {
    if (!g_initialized) {
        fprintf(stderr, "[lodges_LOG ERROR] Log rotate called but not initialized\n");
    ...
    char old_path[512];
    snprintf(old_path, sizeof(old_path), "lodges/logs/kernel/agentos.log");

    char new_path[512];
    snprintf(new_path, sizeof(new_path), "lodges/logs/kernel/agentos_%s.log", timestamp);

    if (rename(old_path, new_path) != 0) {
        fprintf(stderr, "[lodges_LOG ERROR] Failed to rotate log file: %s -> %s\n", old_path, new_path);
        pthread_mutex_unlock(&g_log_lock);
        return lodges_ERR_FILE_OPERATION_FAILED;
    }

    g_main_log_file = fopen(old_path, "a");
    if (!g_main_log_file) {
        fprintf(stderr, "[lodges_LOG ERROR] Failed to create new log file after rotation: %s\n", old_path);
        pthread_mutex_unlock(&g_log_lock);
        return lodges_ERR_FILE_OPEN_FAILED;
    }

    fprintf(stdout, "[lodges_LOG INFO] Log rotated: %s -> %s\n", old_path, new_path);
    pthread_mutex_unlock(&g_log_lock);

    return lodges_SUCCESS;
}

lodges_error_t lodges_log_cleanup(int days_to_keep, uint64_t* freed_bytes) {
    if (!g_initialized) {
        return lodges_ERR_NOT_INITIALIZED;
    }
...
lodges_error_t lodges_log_get_file_info(const char* service, lodges_log_file_info_t* info) {
    if (!info) {
        fprintf(stderr, "[lodges_LOG ERROR] Invalid info parameter (NULL)\n");
        return lodges_ERR_INVALID_PARAM;
    }
    ...
    if (stat(filepath, &st) == 0) {
        ...
        fprintf(stderr, "[lodges_LOG WARN] Failed to get file info: %s\n", filepath);
        return lodges_ERR_FILE_NOT_FOUND;
    }

    return lodges_SUCCESS;
}

lodges_error_t lodges_log_get_stats(uint32_t* total_files, uint64_t* total_size_bytes, time_t* oldest_timestamp) {
