...
#define lodges_SQLITE_IMPLEMENTATION

#ifndef lodges_SQLITE_IMPLEMENTATION

typedef struct {
    lodges_registry_type_t type;
    void* data;
    size_t size;
    void* next;
} registry_node_t;
...
static lodges_error_t init_database(sqlite3* db) {
    const char* sql =
    ...
    if (rc != SQLITE_OK) {
        ...
        return lodges_ERR_DB_INIT_FAILED;
    }
    return lodges_SUCCESS;
}
...
lodges_error_t lodges_registry_init(void) {
    ...
    lodges_error_t err = init_database(g_registry.db);
...
lodges_error_t lodges_registry_add_agent(const lodges_agent_record_t* record) {
    ...
    if (rc != SQLITE_OK) {
        pthread_mutex_unlock(&g_registry.lock);
        return lodges_ERR_DB_QUERY_FAILED;
    }
