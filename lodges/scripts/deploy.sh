...
# lodges Module Deployment Script
...
print_banner() {
    echo "========================================"
    echo "  AgentOS lodges Deployment System"
...
create_directories() {
    ...
    sudo mkdir -p "${DEPLOY_DIR}/include/agentos/lodges"
...
backup_current() {
    ...
    local CURRENT_LIB="${DEPLOY_DIR}/lib/libagentos_lodges.a"
    
    if [ -f "${CURRENT_LIB}" ]; then
        local BACKUP_NAME="lodges_backup_${TIMESTAMP}.tar.gz"
        sudo tar -czvf "${BACKUP_DIR}/${BACKUP_NAME}" \
            -C "${DEPLOY_DIR}" lib/libagentos_lodges.a \
            -C "${DEPLOY_DIR}" include/agentos/lodges/
...
deploy() {
    ...
    # Create version file
    echo "${VERSION}" | sudo tee "${DEPLOY_DIR}/lib/lodges.version"
...
verify() {
    ...
    local LIB_PATH="${DEPLOY_DIR}/lib/libagentos_lodges.a"
    ...
    # Check headers
    if [ ! -d "${DEPLOY_DIR}/include/agentos/lodges" ]; then
        log_error "Header directory not found"
        return 1
    fi
    
    local HEADER_COUNT=$(find "${DEPLOY_DIR}/include/agentos/lodges" -name "*.h" | wc -l)
...
clean_backups() {
    ...
    if [ -d "${BACKUP_DIR}" ]; then
        ...
        ls -t "${BACKUP_DIR}"/lodges_backup_*.tar.gz 2>/dev/null | \
            tail -n +$((KEEP_COUNT + 1)) | \
            xargs -r rm -f
...
health_check() {
    ...
    if [ ! -f "${DEPLOY_DIR}/lib/libagentos_lodges.a" ]; then
    ...
    # Check permissions
    if [ ! -r "${DEPLOY_DIR}/lib/libagentos_lodges.a" ]; then
