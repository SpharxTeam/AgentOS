...
static void test_trace_init_shutdown(void) {
    ...
    lodges_error_t err = lodges_trace_init();
...
static void test_trace_write_batch(void) {
    ...
    err = lodges_trace_write_spans_batch(spans, 5);
    assert(err == lodges_SUCCESS);
...
static void test_trace_invalid_params(void) {
    ...
    err = lodges_trace_write_span(NULL);
    assert(err == lodges_ERR_INVALID_PARAM);

    lodges_span_t invalid_span;
    memset(&invalid_span, 0, sizeof(invalid_span));
    err = lodges_trace_write_span(&invalid_span);
    assert(err == lodges_ERR_INVALID_PARAM);

    err = lodges_trace_write_spans_batch(NULL, 0);
    assert(err == lodges_ERR_INVALID_PARAM);

    err = lodges_trace_write_spans_batch(NULL, 10);
    assert(err == lodges_ERR_INVALID_PARAM);
...
int main(void) {
    printf("=== AgentOS lodges Trace Unit Tests ===\n\n");
