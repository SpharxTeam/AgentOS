...
static void benchmark_init_shutdown(void) {
    ...
    for (int i = 0; i < 100; i++) {
        lodges_init(&(lodges_config_t){.root_path = "bench_lodges"});
        lodges_shutdown();
    }
...
static void benchmark_logging(void) {
    ...
    for (int i = 0; i < BENCHMARK_ITERATIONS; i++) {
        lodges_LOG_ERROR("bench_service", "trace_bench", "Benchmark log message %d", i);
    }
...
static void benchmark_trace_batch_write(void) {
    ...
    for (int batch = 0; batch < BENCHMARK_ITERATIONS / BENCHMARK_BATCH_SIZE; batch++) {
        ...
        lodges_trace_write_spans_batch(spans, BENCHMARK_BATCH_SIZE);
    }
...
static void benchmark_ipc_operations(void) {
    ...
    for (int i = 0; i < 5000; i++) {
        ...
        lodges_ipc_record_channel(&channel);
    }
    ...
    for (int i = 0; i < 5000; i++) {
        ...
        memset(&channel, 0, sizeof(channel));
        snprintf(channel.channel_id, sizeof(channel.channel_id), "bench_ch_%d", i);
        lodges_ipc_get_channel(channel.channel_id, &channel);
    }
...
static void benchmark_memory_operations(void) {
    ...
    for (int i = 0; i < 5000; i++) {
        ...
        lodges_memory_record_pool(&pool);
    }
    ...
    for (int i = 0; i < 5000; i++) {
        ...
        memset(&pool, 0, sizeof(pool));
        snprintf(pool.pool_id, sizeof(pool.pool_id), "bench_pool_%d", i);
        lodges_memory_get_pool(pool.pool_id, &pool);
    }
...
int main(void) {
    printf("===========================================\n");
    printf("   AgentOS lodges Performance Benchmark\n");
    printf("===========================================\n");
