...
import { extractDataMap, getString, getInt64, getMap, getBool, parseTime, getInterfaceSlice } from '../utils';
...
export class SkillManager {
  ...
  async list(opts?: ListOptions): Promise<Skill[]> {
    ...
    return this.parseSkillList(resp);
  }
   ...
   */
  private parseSkillList(resp: APIResponse): Skill[] {
    const data = extractDataMap(resp);
    if (!data) {
      throw new AgentOSError('技能列表响应格式异常', ErrorCode.INVALID_RESPONSE);
    }

    const items = getInterfaceSlice(data, 'skills');
    return items
      .filter((item): item is Record<string, unknown> => typeof item === 'object' && item !== null)
      .map((item) => this.parseSkillFromMap(item, ''));
  }
