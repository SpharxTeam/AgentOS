...
export class TaskManager {
  ...
  async list(opts?: ListOptions): Promise<Task[]> {
    ...
    return this.parseTaskList(resp);
  }
   ...
   */
  private parseTaskList(resp: APIResponse): Task[] {
    const data = extractDataMap(resp);
    if (!data) {
      throw new AgentOSError('任务列表响应格式异常', ErrorCode.INVALID_RESPONSE);
    }

    const items = getInterfaceSlice(data, 'tasks');
    return items
      .filter((item): item is Record<string, unknown> => typeof item === 'object' && item !== null)
      .map((item) => this.parseTaskFromMap(item));
  }
