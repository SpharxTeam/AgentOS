...
`scripts/operations/` 目录包含 AgentOS 系统的运维监控工具，提供性能基准测试、系统健康检查和契约验证等功能。
...
2. 监控系统日志：tail -f agentos/heapstore/logs/*.log
...
# 或赋予目录权限
chmod -R u+rwx agentos/heapstore/logs
```

#### 2. 日志文件不存在

**警告**: `Log file not found: agentos/heapstore/logs/agentos.log`

**解决**:
```bash
# 创建日志目录
mkdir -p agentos/heapstore/logs
touch agentos/heapstore/logs/agentos.log
```
...
```bash
# 设置日志权限
chmod 640 agentos/heapstore/logs/*.log
chown agentos:agentos agentos/heapstore/logs/*.log
```
