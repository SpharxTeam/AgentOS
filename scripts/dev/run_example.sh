...
show_example_info() {
    ...
    if [[ ! -v "EXAMPLES[$example]" ]]; then
        print_status "fail" "未知的示例: $example"
    ...
    if [[ -d "$example_path" ]]; then
        ...
        echo ""
        echo -e "${COLOR_YELLOW}示例目录不存在${COLOR_NC}"
    fi
}
...
run_example() {
    ...
    if [[ ! -d "$example_path" ]]; then
        print_status "fail" "示例目录不存在: $example_path"
        return 1
    fi
    ...
    if [[ -x "$binary" ]]; then
        ...
        if [[ -f "$example_path/config.conf" ]]; then
            AGENTOS_CONFIG="$example_path/config.conf" "$binary"
        else
            "$binary"
        fi
    else
        print_status "warn" "示例无可执行文件"
        echo -e "${COLOR_DIM}请手动执行: cd $example_path && make${COLOR_NC}"
    fi
}

build_examples() {
    print_section "构建所有示例"
