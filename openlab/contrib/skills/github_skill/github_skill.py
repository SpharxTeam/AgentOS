...
This module provides GitHub integration capabilities for the openlab platform,
...
class GitHubSkill:
    ...
    def _parse_config(self, manager: Dict[str, Any]) -> GitHubConfig:
        """Parse configuration dictionary."""
        return GitHubConfig(
            github_token=manager.get("github_token", ""),
            organization=manager.get("organization"),
            default_branch=manager.get("default_branch", "main"),
            api_base_url=manager.get("api_base_url", "https://api.github.com"),
            timeout=manager.get("timeout", 30),
            per_page=manager.get("per_page", 100)
        )

    def initialize(self) -> GitHubResult:
        """Initialize GitHub API connection."""
        if not self.manager.github_token:
        ...
        if PYTHON_GITHUB_AVAILABLE:
            ...
            self.session.headers.update({
                "Authorization": f"token {self.manager.github_token}",
    ...
    def get_user(self) -> GitHubResult:
        ...
        try:
            if self.github:
                ...
                response = self.session.get(f"{self.manager.api_base_url}/user")
    ...
    def get_repository(self, owner: str, repo: str) -> GitHubResult:
        ...
        try:
            if self.github:
                ...
                response = self.session.get(f"{self.manager.api_base_url}/repos/{owner}/{repo}")
                self._update_rate_limit(response.headers)
                if response.status_code == 200:
                    return GitHubResult(success=True, data=response.json(), status_code=200)
                return GitHubResult(success=False, error=response.text, status_code=response.status_code)
        ...
        try:
            if self.github:
                ...
                response = self.session.post(
                    f"{self.manager.api_base_url}/user/repos",
                    json=data
                )
                self._update_rate_limit(response.headers)
    ...
    def delete_repository(self, owner: str, repo: str) -> GitHubResult:
        ...
        try:
            if self.github:
                ...
                response = self.session.delete(f"{self.manager.api_base_url}/repos/{owner}/{repo}")
                self._update_rate_limit(response.headers)
    ...
    def list_repositories(self, per_page: Optional[int] = None, sort: str = "updated") -> GitHubResult:
        ...
        per_page = per_page or self.manager.per_page
    ...
    def create_issue(
        ...
        try:
            if self.github:
                ...
                response = self.session.post(
                    f"{self.manager.api_base_url}/repos/{owner}/{repo}/issues",
                    json=data
                )
                self._update_rate_limit(response.headers)
                if response.status_code in [200, 201]:
    ...
    def get_issue(self, owner: str, repo: str, issue_number: int) -> GitHubResult:
        ...
        try:
            if self.github:
                ...
                response = self.session.get(
                    f"{self.manager.api_base_url}/repos/{owner}/{repo}/issues/{issue_number}"
                )
                self._update_rate_limit(response.headers)
                if response.status_code == 200:
                    return GitHubResult(success=True, data=response.json(), status_code=200)
    ...
    def create_pull_request(
        ...
        base = base or self.manager.default_branch
        ...
        try:
            if self.github:
                ...
                response = self.session.post(
                    f"{self.manager.api_base_url}/repos/{owner}/{repo}/pulls",
                    json=data
                )
                self._update_rate_limit(response.headers)
                if response.status_code in [200, 201]:
    ...
    def get_pull_request(self, owner: str, repo: str, pr_number: int) -> GitHubResult:
        ...
        try:
            if self.github:
                ...
                response = self.session.get(
                    f"{self.manager.api_base_url}/repos/{owner}/{repo}/pulls/{pr_number}"
                )
                self._update_rate_limit(response.headers)
                if response.status_code == 200:
                    return GitHubResult(success=True, data=response.json(), status_code=200)
        ...
        try:
            if self.github:
                ...
                response = self.session.put(
                    f"{self.manager.api_base_url}/repos/{owner}/{repo}/pulls/{pr_number}/merge",
                    json=data
                )
                self._update_rate_limit(response.headers)
                if response.status_code == 200:
    ...
    def create_branch(self, owner: str, repo: str, branch_name: str, from_branch: Optional[str] = None) -> GitHubResult:
        ...
        from_branch = from_branch or self.manager.default_branch
        ...
        try:
            if self.github:
                ...
                ref_response = self.session.get(
                    f"{self.manager.api_base_url}/repos/{owner}/{repo}/git/refs/heads/{from_branch}"
                )
                ...
                response = self.session.post(
                    f"{self.manager.api_base_url}/repos/{owner}/{repo}/git/refs",
                    json=data
                )
                self._update_rate_limit(response.headers)
                if response.status_code in [200, 201]:
        ...
        try:
            if self.github:
                ...
                response = self.session.delete(
                    f"{self.manager.api_base_url}/repos/{owner}/{repo}/git/refs/heads/{branch_name}"
                )
                self._update_rate_limit(response.headers)
                if response.status_code == 204:
    ...
    def create_release(
        ...
        try:
            if self.github:
                ...
                response = self.session.post(
                    f"{self.manager.api_base_url}/repos/{owner}/{repo}/releases",
                    json=data
                )
                self._update_rate_limit(response.headers)
                if response.status_code in [200, 201]:
    ...
    def dispatch_workflow(
        ...
        try:
            if self.github:
                ...
                response = self.session.post(
                    f"{self.manager.api_base_url}/repos/{owner}/{repo}/actions/workflows/{workflow_id}/dispatches",
                    json=data
                )
                self._update_rate_limit(response.headers)
                if response.status_code == 204:
    ...
    def get_workflow_runs(
        ...
        try:
            if self.github:
                ...
                response = self.session.get(
                    f"{self.manager.api_base_url}/repos/{owner}/{repo}/actions/workflows/{workflow_id}/runs",
                    params=params
                )
                self._update_rate_limit(response.headers)
                if response.status_code == 200:
    ...
    def search_code(self, query: str, per_page: Optional[int] = None) -> GitHubResult:
        ...
        try:
            if self.github:
                ...
                response = self.session.get(
                    f"{self.manager.api_base_url}/search/code",
    ...
    def search_repositories(self, query: str, sort: Optional[str] = None, per_page: Optional[int] = None) -> GitHubResult:
        ...
        try:
            if self.github:
                ...
                response = self.session.get(
                    f"{self.manager.api_base_url}/search/repositories",
                    params=params
                )
                self._update_rate_limit(response.headers)
                if response.status_code == 200:
    ...
    def get_file_content(self, owner: str, repo: str, path: str, ref: Optional[str] = None) -> GitHubResult:
        ...
        try:
            if self.github:
                ...
                url = f"{self.manager.api_base_url}/repos/{owner}/{repo}/contents/{path}"
                if ref:
                    url += f"?ref={ref}"
    ...
    def add_collaborator(self, owner: str, repo: str, username: str, permission: str = "push") -> GitHubResult:
        ...
        try:
            if self.github:
                ...
                response = self.session.put(
                    f"{self.manager.api_base_url}/repos/{owner}/{repo}/collaborators/{username}",
                    json=data
                )
                self._update_rate_limit(response.headers)
    ...
    def get_rate_limit(self) -> GitHubResult:
        ...
        try:
            if self.github:
                ...
                response = self.session.get(f"{self.manager.api_base_url}/rate_limit")
                if response.status_code == 200:
...
def create_skill(manager: Optional[Dict[str, Any]] = None) -> GitHubSkill:
    """Factory function to create a GitHubSkill instance."""
    return GitHubSkill(manager)
