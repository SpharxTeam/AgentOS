...
typedef struct {
    ...
    double input_cost_per_1k;         /**< 输入成本（美元/1K Token） */
    double output_cost_per_1k;         /**< 输出成本（美元/1K Token） */
...
agentos_cost_estimator_t* agentos_cost_estimator_create(const char* config_path) {
    ...
    agentos_cost_estimator_t* estimator = (agentos_cost_estimator_t*)malloc(
...
double agentos_cost_estimator_estimate(agentos_cost_estimator_t* estimator, 
    ...
    const model_cost_config_t* config = find_model_config(estimator, normalized);
    if (!config) {
        return -1.0;
    }
    
    double input_cost = (input_tokens / 1000.0) * config->input_cost_per_1k;
    double output_cost = (output_tokens / 1000.0) * config->output_cost_per_1k;
