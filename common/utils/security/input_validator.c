...
agentos_error_t agentos_sanitize_shell_param(
    ...
    char* sanitized = (char*)malloc(buf_size);
...
agentos_error_t agentos_sanitize_sql_identifier(
    ...
    char* sanitized = (char*)malloc(len + 3);
    if (!sanitized) return AGENTOS_ENOMEM;
...
agentos_error_t agentos_parse_url(
    ...
    if (out_scheme) {
        *out_scheme = (char*)malloc(scheme_len + 1);
    ...
    if (out_host) {
        *out_host = (char*)malloc(host_len + 1);
        if (!*out_host) {
            if (out_scheme && *out_scheme) free(*out_scheme);
    ...
    if (out_path) {
        ...
        *out_path = (char*)malloc(path_len + 1);
        if (!*out_path) {
            if (out_scheme && *out_scheme) free(*out_scheme);
            if (out_host && *out_host) free(*out_host);
            return AGENTOS_ENOMEM;
        }
