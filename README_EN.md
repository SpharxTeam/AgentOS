...
**Architecture Design** · Complete technology stack from kernel to application.

```
Application Layer (openlab)
    ↕
Service Layer (daemon) — gateway_d · llm_d · market_d · monit_d · sched_d · tool_d
    ↕
Kernel Layer (atoms) — corekern · coreloopthree · memoryrovol · syscall
    ↕
Security Layer (cupolas) — Sanitization / Permission / Audit / Sandbox
    ↕
Support Layer (commons) — platform · utils (19 modules)
    ↕
...
| Python | `pip install agentos` then import directly |
| Go | `import "github.com/spharx/agentos/toolkit/go"` |
| Rust | `use agentos_toolkit::prelude::*;` |
| TypeScript | `npm install @spharx/agentos-toolkit` then import directly |
