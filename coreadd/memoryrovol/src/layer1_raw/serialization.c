...
#include <string.h>
#include <stdio.h>
#include <cjson/cJSON.h>  // 使用 cJSON 库

/**
 * @brief 将元数据对象序列化为 JSON 字符串
 * @param meta 元数据指针
...
agentos_raw_metadata_t* agentos_raw_metadata_from_json(const char* json) {
    ...
    if (item && cJSON_IsString(item)) meta->record_id = strdup(item->valuestring);
    ...
    if (item && cJSON_IsString(item)) meta->source = strdup(item->valuestring);

    item = cJSON_GetObjectItem(root, "trace_id");
    if (item && cJSON_IsString(item)) meta->trace_id = strdup(item->valuestring);
...
agentos_error_t agentos_layer1_raw_deserialize(
    ...
    char* meta_json_copy = (char*)malloc(meta_len + 1);
