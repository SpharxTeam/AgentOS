...
```bash
# 安装最新版本
npm install @agentos/manuals-sdk

# 或使用 yarn
yarn add @agentos/manuals-sdk

# 或使用 pnpm
pnpm add @agentos/manuals-sdk
```
...
```html
<!-- 在浏览器中使用 -->
<script src="https://cdn.jsdelivr.net/npm/@agentos/manuals-agentos/toolkit/dist/browser/manuals.min.js"></script>
```
...
```typescript
import { PartdocsClient } from '@agentos/manuals-sdk';
...
```typescript
import { DocumentFormat, DocumentStatus } from '@agentos/manuals-sdk';

async function createFirstDocument() {
  try {
    const document = await client.documents.create({
      title: '我的第一个文档',
      content: '# 欢迎使用 AgentOS manuals\n\n这是使用 TypeScript SDK 创建的第一个文档。',
    ...
    console.log('文档链接:', `${client.manager.baseUrl}/documents/${document.slug}`);
  } catch (error) {
...
```typescript
import { ValidationSeverity } from '@agentos/manuals-sdk';
...
```typescript
import { RealtimeClient } from '@agentos/manuals-agentos/toolkit/realtime';
...
import { 
  ...
  RateLimitError,
  ServerError 
} from '@agentos/manuals-agentos/toolkit/errors';
...
```typescript
import { RetryConfig, withRetry } from '@agentos/manuals-agentos/toolkit/utils';
...
client.interceptors.request.use(
  (manager) => {
    // 修改请求配置
    manager.headers['X-Custom-Header'] = 'custom-value';
    manager.headers['X-Request-ID'] = generateRequestId();
    
    // 记录请求日志
    console.log('发送请求:', manager.method, manager.url);
...
client.interceptors.response.use(
  (response) => {
    // 处理响应数据
    console.log('收到响应:', response.status, response.manager.url);
...
```typescript
import { PartdocsClient, ClientConfig } from '@agentos/manuals-sdk';

const manager: ClientConfig = {
  // 必需配置
  baseUrl: 'http://localhost:8000',
  apiKey: 'your-api-key',
...
const client = new PartdocsClient(manager);
```
...
```typescript
// 从环境变量创建客户端
import { loadConfigFromEnv } from '@agentos/manuals-agentos/toolkit/manager';

const manager = loadConfigFromEnv();
const client = new PartdocsClient(manager);
```
...
// 3. 使用流式处理
import { createDocumentStream } from '@agentos/manuals-agentos/toolkit/streams';
...
```typescript
// 1. 安全存储API密钥
import { SecureStorage } from '@agentos/manuals-agentos/toolkit/security';
...
// 2. 输入验证
import { validateDocumentInput } from '@agentos/manuals-agentos/toolkit/validation';
...
program
  .name('manuals-cli')
  .description('AgentOS manuals 命令行工具')
  .version('1.0.0');
...
import { PartdocsClient, Document, DocumentFormat } from '@agentos/manuals-sdk';
...
// 添加请求/响应拦截器进行调试
client.interceptors.request.use(manager => {
  console.debug('请求:', {
...
```typescript
// 获取SDK版本信息
import { version } from '@agentos/manuals-agentos/toolkit/package.json';
console.log('SDK版本:', version);

// 获取客户端信息
console.log('客户端配置:', client.manager);
...
### 12.1 官方资源
- [GitHub仓库](https://github.com/agentos/manuals-sdk-typescript)
- [API参考文档](https://docs.agentos.dev/api/typescript)
- [示例代码](https://github.com/agentos/manuals-sdk-examples)

### 12.2 社区资源
- [Discord社区](https://discord.gg/agentos)
- [Stack Overflow标签](https://stackoverflow.com/questions/tagged/agentos-manuals)
- [博客教程](https://blog.agentos.dev/tag/manuals)

### 12.3 工具集成
- [VS Code扩展](https://marketplace.visualstudio.com/items?itemName=agentos.manuals)
- [WebStorm插件](https://plugins.jetbrains.com/plugin/12345-agentos-manuals)
- [CLI工具](https://www.npmjs.com/package/@agentos/manuals-cli)
