 ...
 * @brief L1 原始卷异步存储引擎实现
 * @copyright (c) 2026 SPHARX. All Rights Reserved.
 * 
 * @details
 * L1 原始卷异步存储引擎提供高性能、高可靠的异步数据存储服务。
 * 支持批量写入、流控制、错误重试和容灾恢复，达到99.999%生产级可靠性标准。
 * 
 * 核心功能：
...
typedef struct async_operation {
    ...
    agentos_error_t* out_error;    /**< 输出错误码 */
    agentos_semaphore_t* semaphore; /**< 同步信号量 */
    int retry_count;               /**< 重试次数 */
    uint64_t timestamp_ns;         /**< 时间戳 */
    struct async_operation* next;  /**< 下一个操作 */
} async_operation_t;
...
typedef struct async_queue {
    async_operation_t* head;       /**< 队列头 */
    async_operation_t* tail;       /**< 队列尾 */
    size_t size;                   /**< 当前大小 */
    size_t capacity;               /**< 最大容量 */
    agentos_mutex_t* lock;         /**< 队列锁 */
    agentos_semaphore_t* semaphore; /**< 信号量 */
} async_queue_t;
...
struct agentos_layer1_raw {
    ...
    uint64_t total_delete_count;   /**< 总删除次数 */
    ...
    size_t batch_count;             /**< 批处理计数 */
    uint64_t last_flush_time_ns;    /**< 最后刷新时间 */
...
agentos_error_t agentos_layer1_raw_create_async(
    ...
    l1->workers = (worker_thread_t*)calloc(workers, sizeof(worker_thread_t));
    if (!l1->workers) {
    ...
    l1->running = 1;
    l1->healthy = 1;
    l1->health_message = strdup("Initializing");
    ...
    // 更新健康状态
    free(l1->health_message);
    l1->health_message = strdup("Running");
    l1->last_health_check_ns = agentos_get_monotonic_time_ns();
...
void agentos_layer1_raw_destroy(agentos_layer1_raw_t* l1) {
    ...
    // 清理资源
    if (l1->storage_path) free(l1->storage_path);
    if (l1->lock) agentos_mutex_destroy(l1->lock);
    if (l1->obs) agentos_observability_destroy(l1->obs);
    if (l1->health_message) free(l1->health_message);
    ...
    do {
        if (!(find_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
            ...
            if (len > 4 && strcmp(filename + len - 4, ".raw") == 0) {
                filename[len - 4] = '\0';
                ids[index] = strdup(filename);
                if (!ids[index]) {
                    // 清理已分配的内存
                    for (size_t i = 0; i < index; i++) free(ids[i]);
                    free(ids);
                    FindClose(hFind);
                    return AGENTOS_ENOMEM;
                }
