...
typedef struct layer_monitoring_data {
    ...
    uint64_t last_cleanup_ns;      /**< 最后清理时间 */
...
typedef struct retrieval_monitoring_data {
    ...
    double avg_precision;          /**< 平均精确率 */
    double avg_recall;             /**< 平均召回率 */
...
typedef struct alert_rule {
    ...
    uint64_t last_triggered_ns;    /**< 最后触发时间 */
...
struct agentos_memoryrov_monitor {
    ...
    /* 分层监控数据 */
    layer_monitoring_data_t l1_monitoring;   /**< L1原始层监控 */
    layer_monitoring_data_t l2_monitoring;   /**< L2特征层监控 */
    layer_monitoring_data_t l3_monitoring;   /**< L3结构层监控 */
    layer_monitoring_data_t l4_monitoring;   /**< L4模式层监控 */
    
    /* 子系统监控数据 */
    retrieval_monitoring_data_t retrieval_monitoring;   /**< 检索监控 */
    ...
    uint64_t last_export_ns;                 /**< 最后导出时间 */
};
...
static monitoring_metric_t* create_metric(const char* name, metric_type_t type, 
    ...
    monitoring_metric_t* metric = (monitoring_metric_t*)calloc(1, sizeof(monitoring_metric_t));
    if (!metric) {
    ...
    if (!metric->name || (description && !metric->description) || (unit && !metric->unit)) {
        if (metric->name) free(metric->name);
        if (metric->description) free(metric->description);
        if (metric->unit) free(metric->unit);
        free(metric);
...
agentos_error_t agentos_memoryrov_monitor_create(agentos_memoryrov_monitor_t** out_monitor) {
    ...
    agentos_memoryrov_monitor_t* monitor = (agentos_memoryrov_monitor_t*)calloc(1, sizeof(agentos_memoryrov_monitor_t));
    if (!monitor) {
    ...
    if (!monitor->monitor_id) {
        ...
        monitor->monitor_id = strdup("memoryrov_monitor_default");
    }
    ...
    if (monitor->obs) {
        ...
        agentos_observability_register_metric(monitor->obs, "memoryrov_operations_total", 
                                              AGENTOS_METRIC_COUNTER, "MemoryRovol总操作次数");
        agentos_observability_register_metric(monitor->obs, "memoryrov_errors_total", 
                                              AGENTOS_METRIC_COUNTER, "MemoryRovol总错误次数");
...
void agentos_memoryrov_monitor_destroy(agentos_memoryrov_monitor_t* monitor) {
    ...
    while (rule) {
        ...
        if (rule->metric_name) free(rule->metric_name);
        if (rule->condition) free(rule->condition);
        if (rule->severity) free(rule->severity);
        if (rule->message_template) free(rule->message_template);
        free(rule);
        rule = next;
    }
 ...
 * @brief 记录L1层操作
 * @param monitor 监控器
 * @param operation 操作类型（0=写入，1=读取，2=删除）
 * @param bytes 字节数
 ...
 * @brief 记录L2层操作
 * @param monitor 监控器
 * @param operation 操作类型（0=添加向量，1=查询向量，2=删除向量）
 * @param vector_count 向量数量
 * @param duration_ns 耗时（纳秒）
 * @param success 是否成功
 * @return agentos_error_t
