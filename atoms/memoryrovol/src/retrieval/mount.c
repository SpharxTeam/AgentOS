...
agentos_error_t agentos_mounter_create(
    ...
    agentos_mounter_t* m = (agentos_mounter_t*)calloc(1, sizeof(agentos_mounter_t));
    if (!m) {
...
agentos_error_t agentos_mounter_mount(
    ...
    // 添加到挂载链表
    mount_entry_t* entry = (mount_entry_t*)malloc(sizeof(mount_entry_t));
    if (!entry) {
        agentos_mutex_unlock(mounter->lock);
        free(text);
        return AGENTOS_ENOMEM;
    ...
    *out_content = strdup(entry->content);
    return AGENTOS_SUCCESS;
}
