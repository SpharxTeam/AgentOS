...
typedef struct token_bucket {
    ...
    uint64_t tokens;                /**< 当前令牌�?*/
    uint64_t refill_rate;           /**< 补充速率（每秒） */
    uint64_t last_refill_ns;        /**< 最后补充时�?*/
} token_bucket_t;
...
typedef struct sliding_window {
    ...
    window_entry_t* head;           /**< 头指�?*/
    window_entry_t* tail;           /**< 尾指�?*/
    uint64_t count;                 /**< 当前计数 */
...
typedef struct fixed_window {
    ...
    uint64_t window_start_ns;       /**< 窗口开始时�?*/
} fixed_window_t;
...
typedef struct rate_limiter_stats {
    ...
    uint64_t allowed_requests;      /**< 允许请求�?*/
    uint64_t rejected_requests;     /**< 拒绝请求�?*/
    uint64_t current_usage;         /**< 当前使用�?*/
    uint64_t peak_usage;            /**< 峰值使�?*/
    uint64_t wait_time_ns;          /**< 总等待时�?*/
    uint64_t last_reject_ns;        /**< 最后拒绝时�?*/
