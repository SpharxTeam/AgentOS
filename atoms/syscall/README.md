...
Syscall（系统调用）是用户态服务与内核之间的**唯一通信通道**。所有守护进程（`backs/`）必须通过 syscall 与内核交互，禁止直接调用内核内部函数。
...
- [系统调用规范](partdocs/architecture/folder/syscall.md)
- [微内核设计](partdocs/architecture/folder/microkernel.md)
- [IPC 通信机制](partdocs/architecture/folder/ipc.md)
- [统一日志系统](partdocs/architecture/folder/logging_system.md)
