...
// 初始化模块
cupolas_init("/etc/agentos/cupolas/config.yaml", NULL);
...
// 加载权限声明
int ret = cupolas_entitlements_load("/etc/agentos/cupolas/entitlements.yaml");
...
int main(int argc, char* argv[]) {
    ...
    ret = cupolas_init("/etc/agentos/cupolas/config.yaml", &error);
    if (ret != 0) {
...
gcc -o example example.c \
  -I/path/to/agentos/cupolas/include \
  -L/path/to/agentos/cupolas/build \
  -lcupolas \
  -lssl -lcrypto \
  -lyaml -lcjson
```
...
**位置**: `/etc/agentos/cupolas/config.yaml`
...
# 权限模块配置
permission:
  rules_file: "/etc/agentos/cupolas/permission_rules.yaml"
  cache_size: 10000
...
# 净化器配置
sanitizer:
  rules_file: "/etc/agentos/cupolas/sanitizer_rules.yaml"
...
security:
  ...
  vault:
    enable: true
    storage_path: "/var/lib/agentos/cupolas/vault"
    enable_audit: true
  ...
  entitlements:
    enable: true
    rules_file: "/etc/agentos/cupolas/entitlements.yaml"
...
**A**: 默认存储在 `/var/log/agentos/cupolas/`，可通过配置文件修改。
...
- [C 语言编码规范](../../agentos/manuals/specifications/coding_standard/C_coding_style_guide.md)
