...
`sanitizer/` 是 cupolas 安全沙箱的**输入验证和过滤层**，防止恶意输入和注入攻击。
...
sanitizer_t* sanitizer = sanitizer_create(
    "./agentos/manager/sanitizer_rules.json"  // 规则文件
);
...
int main() {
    // 1. 初始化 cupolas（包含 sanitizer）
    domes_config_t manager = {
        .sanitizer_enabled = true,
        .sanitizer_rules_path = "./agentos/manager/sanitizer_rules.json",
    ...
    domes_sanitizer_add_rule(cupolas, &custom_rule);
...
domes_sanitizer_add_rule(cupolas, &sensitive_word_rule);
...
manager.cache_size = 512;  // 512 条缓存
...
- [输入验证规范](../../../paper/specifications/input_validation.md)
