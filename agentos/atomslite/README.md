...
`agentos/atomslite/` 是 AgentOS 的**精简版内核实现**，专为资源受限环境设计。相比完整的 `agentos/atoms/` 模块，本模块提供：

- **轻量化核心** (`corekernlite/`): 仅包含最基础的 IPC 和内存管理机制
- **裁剪的设计**: 去除复杂调度器和大容量记忆系统，保持核心功能
- **嵌入式友好**: 适用于 IoT 设备、边缘计算、移动设备等资源受限场景
- **API 兼容**: 与完整版 `agentos/atoms/` 保持接口一致性，便于迁移和升级
...
- ✅ 功能单一的专用智能体（单一任务场景）
- ✅ 边缘计算节点（本地预处理和决策）
- ❌ 需要复杂认知的大型智能体（请使用 `agentos/atoms/`）
- ❌ 多智能体协作场景（需要完整运行时）
- ❌ 需要长期记忆和模式学习的应用
...
**注意**: `agentos/atomslite/` 不依赖 FAISS、SQLite3 等重型库，仅需标准 C 库。
...
1. **复杂认知智能体** → 使用 `agentos/atoms/` 的 `coreloopthree/` 模块
2. **多智能体协作** → 使用 `agentos/atoms/` 的完整 IPC 和会话管理
3. **需要长期记忆** → 使用 `agentos/atoms/memoryrovol/` 的四层记忆系统
4. **大规模数据处理** → 使用 `agentos/atoms/` 的高性能记忆检索
...
- 📚 [完整内核层 agentos/atoms/](../agentos/atoms/README.md) - 完整版微内核实现
- 🏛️ [微内核基础 corekern](../agentos/atoms/corekern/README.md) - 微内核详细设计
- 🔌 [系统调用层](../agentos/atoms/syscall/README.md) - 系统调用规范
