...
async_queue_t* async_queue_create(size_t capacity) {
    ...
    if (ret != 0) {
        AGENTOS_LOG_ERROR("Failed to init mutex: %d", ret);
        AGENTOS_FREE(queue);
        return NULL;
    }
    ...
    if (ret != 0) {
        AGENTOS_LOG_ERROR("Failed to init cond not_empty: %d", ret);
    ...
    if (ret != 0) {
        AGENTOS_LOG_ERROR("Failed to init cond not_full: %d", ret);
