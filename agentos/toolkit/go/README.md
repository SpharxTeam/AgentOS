...
├── manager.go           # 配置管理（函数式选项模式、环境变量注入、零值合并）
...
```bash
go get github.com/spharx/agentos/toolkit/go
```
...
```go
// 从环境变量创建配置
manager := agentos.NewConfigFromEnv()
client, err := agentos.NewClientWithConfig(manager)
```
...
| `NewClientWithConfig(manager *manager)` | 使用预构建配置创建 |
| `Tasks()` | 获取任务管理器 |
| `Memories()` | 获取记忆管理器 |
| `Sessions()` | 获取会话管理器 |
| `Skills()` | 获取技能管理器 |
