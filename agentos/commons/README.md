...
`agentos/commons/` 是 AgentOS 的**统一基础库**，提供跨模块共享的通用工具和基础设施能力。作为整个系统的底层依赖，commons 为 atoms（内核层）、daemon（服务层）、cupolas（安全层）等所有上层模块提供标准化的基础服务。
