...
the OpenHub platform. It analyzes complex tasks, breaks them into manageable
...
class PlanningStrategy:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.max_subtasks = config.get("max_subtasks", 20)
        self.complexity_threshold = config.get("complexity_threshold", 7.0)
        self.parallel_execution = config.get("parallel_execution", True)
        self.risk_aware = config.get("risk_aware", True)
        self.optimization_level = config.get("optimization_level", 2)
