...
This module provides validation for Agent contracts in the OpenHub Market.
...
class AgentContractValidator:
    """
    Validator for Agent contracts in the OpenHub Market.
    ...
    def _validate_semantics(self, contract_dict: Dict[str, Any]) -> List[ValidationIssue]:
        ...
        if config_schema:
            if config_schema.get("type") != "object":
                issues.append(ValidationIssue(
                    severity=ValidationSeverity.WARNING,
                    message="Config schema type should be 'object' for better compatibility",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate Agent contracts for OpenHub Market"
    )
    parser.add_argument(
        "contract_file",
