...
Command-line interface for managing agents in the OpenHub Market.
...
class AgentCLI:
    ...
    in the OpenHub Market, with support for multiple output formats
    ...
    def _print_list(self, data: List[Any], title: Optional[str] = None) -> None:
        ...
        if self.console and RICH_AVAILABLE:
            ...
            if all(isinstance(item, dict) for item in data):
                ...
                # Add rows
                for item in data:
                    table.add_row(*[str(item.get(key, "")) for key in first_item.keys()])
                
                self.console.print(table)
            else:
                # Simple list
                if title:
                    self.console.print(Panel("\n".join(str(item) for item in data), title=title))
                else:
                    for item in data:
    ...
    async def install(
        ...
        try:
            ...
            if result.success:
                if validate_only:
                    self.print_success(f"Agent contract is valid: {result.agent_id} version {result.version}")
                else:
                    self.print_success(f"Installed agent: {result.agent_id} version {result.version}")
                    self.print_info(f"Installation path: {result.install_path}")
    ...
    async def uninstall(
        ...
        try:
            async with AgentInstaller(self.install_root) as installer:
                if self.verbose:
                    if version:
                        self.print_info(f"Uninstalling agent: {agent_id} version {version}")
                    else:
                        self.print_info(f"Uninstalling all versions of agent: {agent_id}")
                
                success = await installer.uninstall(agent_id, version, force)
                
                if success:
                    if version:
                        self.print_success(f"Uninstalled agent: {agent_id} version {version}")
                    else:
                        self.print_success(f"Uninstalled all versions of agent: {agent_id}")
                    return 0
                else:
        ...
        try:
            async with AgentInstaller(self.install_root) as installer:
                ...
                if detailed:
                    ...
                    self.print_result(detailed_agents, "Installed Agents (Detailed)")
                else:
                    # Show summary
                    summary = []
                    for agent in agents:
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="OpenHub Agent Management CLI",
...
Examples:
  ...
  %(prog)s validate ./agent_contract.json --strict
    ...
    parser.add_argument(
        "--install-root",
        help="Root directory for agent installations (default: ~/.openhub/agents)"
    )
    parser.add_argument(
        "--format",
