...
This module provides the main FastAPI application for the OpenHub Video Editing
...
class FilterRequest:
    ...
    filter_type: str = Field(..., description="Filter type: brightness, contrast, saturation, blur, grayscale, sepia")
    filter_value: float = Field(default=50.0, ge=0, le=100, description="Filter intensity value")
    output_format: str = Field(default=".mp4", description="Output format")
...
class ConvertRequest:
    ...
    output_format: str = Field(..., description="Target format: mp4, avi, mov, mkv, webm, gif")
    video_codec: Optional[str] = Field(None, description="Video codec override")
...
class GifRequest:
    ...
    start_time: float = Field(..., ge=0, description="Start time in seconds")
    duration: float = Field(..., gt=0, le=30, description="GIF duration in seconds")
...
class AudioExtractRequest:
    ...
    output_format: str = Field(default="mp3", description="Audio format: mp3, aac, wav")


@dataclass
class SubtitleRequest:
    """Request model for adding subtitles."""
    subtitle_path: str = Field(..., description="Path to subtitle file (SRT format)")
    output_format: str = Field(default=".mp4", description="Output format")
...
class VideoEditApp:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.pipeline = EditPipeline(self.config._config or self.config._default_config())
    ...
    def _setup_fastapi(self):
        ...
        self.app = FastAPI(
            title="OpenHub Video Editing Platform",
            description="A comprehensive video editing solution for the OpenHub marketplace",
            version="1.0.0",
            lifespan=lifespan
        )
    ...
    def _setup_routes(self):
        ...
        async def root():
            """Root endpoint."""
            return {
                "service": "OpenHub Video Editing Platform",
                "version": "1.0.0",
                "status": "operational"
            }
        ...
        async def upload_video(file: UploadFile = File(...)):
            ...
            try:
                ...
                supported = self.config.get("supported_formats", {}).get("input", [])
                ...
                max_size = self.config.get("max_file_size", 5368709120)
        ...
        async def get_supported_formats():
            ...
            return {
                "input": self.config.get("supported_formats", {}).get("input", []),
                "output": self.config.get("supported_formats", {}).get("output", []),
        ...
        async def cleanup_temp_files(hours: Optional[int] = None):
            ...
            cleanup_hours = hours or self.config.get("tasks.auto_cleanup_hours", 24)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="OpenHub Video Editing Platform")
    ...
    app = VideoEditApp(config_path=args.config)
