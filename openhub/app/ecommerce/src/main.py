...
This module provides the main FastAPI application for the OpenHub E-commerce
...
class EcommerceApp:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        db_url = self.config.get("database_url", "sqlite:///./ecommerce.db")
        redis_url = self.config.get("redis_url", "redis://localhost:6379")
        stripe_key = self.config.get("stripe_secret_key", "")
        jwt_secret = self.config.get("jwt_secret_key", "change-me")
        jwt_algorithm = self.config.get("jwt_algorithm", "HS256")
        jwt_expire = self.config.get("jwt_access_token_expire_minutes", 30)
    ...
    def _setup_fastapi(self):
        ...
        self.app = FastAPI(
            title="OpenHub E-commerce Platform",
            description="A comprehensive e-commerce solution for the OpenHub marketplace",
            version="1.0.0",
            lifespan=lifespan
        )
    ...
    def _setup_routes(self):
        ...
        async def root():
            """Root endpoint."""
            return {
                "service": "OpenHub E-commerce Platform",
                "version": "1.0.0",
                "status": "operational"
            }
        ...
        async def stripe_webhook(request: Request):
            ...
            try:
                ...
                event = self.payment._client.Webhook.construct_event(
                    payload, sig, self.config.get("stripe_webhook_secret", "")
                )
        ...
        async def get_shipping_rates(weight: float, destination: Optional[str] = None):
            ...
            try:
                ...
                if not result:
                    default_rate = self.config.get("products.default_shipping_cost", 5.99)
                    result.append({
                        "carrier": "Standard",
                        "service": "Ground",
                        "cost": default_rate,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="OpenHub E-commerce Platform")
    ...
    app = EcommerceApp(config_path=args.config)
