...
static struct {
    int initialized;
    domes_runtime_protect_config_t manager;
...
int domes_runtime_protect_init(const domes_runtime_protect_config_t* manager) {
    ...
    if (manager) {
        ...
        g_runtime_protect.manager.level = DOMES_PROTECT_BASIC;
...
int domes_runtime_protect_enable(const domes_runtime_protect_config_t* manager) {
    if (!g_runtime_protect.initialized) {
        int result = domes_runtime_protect_init(manager);
    ...
    if (g_runtime_protect.manager.memory.enable_dep ||
        ...
        int result = domes_memory_protect_enable(&g_runtime_protect.manager.memory);
        if (result != 0) return result;
    }
    
    if (g_runtime_protect.manager.cfi.enable_cfi) {
        int result = domes_cfi_enable(&g_runtime_protect.manager.cfi);
        if (result != 0) return result;
    }
    
    if (g_runtime_protect.manager.seccomp.enable_seccomp) {
        int result = domes_seccomp_enable(&g_runtime_protect.manager.seccomp);
        if (result != 0) return result;
    }
    
    if (g_runtime_protect.manager.integrity.enable_code_integrity ||
        g_runtime_protect.manager.integrity.enable_data_integrity) {
        int result = domes_integrity_enable(&g_runtime_protect.manager.integrity);
        if (result != 0) return result;
    }
...
int domes_seccomp_enable(const domes_seccomp_config_t* manager) {
    ...
    if (manager->allowed_syscalls) {
        for (size_t i = 0; i < manager->syscall_count && 
            ...
            rule->syscall_name = domes_strdup(manager->allowed_syscalls[i]);
            rule->action = 0;
            rule->arg_index = 0;
            rule->arg_value = 0;
            rule->op[0] = '\0';
