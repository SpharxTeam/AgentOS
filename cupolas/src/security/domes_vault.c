...
typedef struct {
    char* cred_id;
    domes_vault_cred_type_t type;
    uint8_t* encrypted_data;
    size_t encrypted_len;
    uint8_t iv[AES_IV_SIZE];
    uint8_t salt[SALT_SIZE];
    domes_vault_acl_t acl;
    domes_vault_metadata_t metadata;
} credential_entry_t;
...
struct domes_vault {
    ...
    domes_rwlock_t lock;
    domes_vault_config_t manager;
};

typedef struct {
    bool initialized;
    domes_vault_config_t default_config;
    domes_rwlock_t global_lock;
} vault_global_ctx_t;
...
int domes_vault_init(const domes_vault_config_t* manager) {
    ...
    if (manager) {
        memcpy(&g_vault_ctx.default_config, manager, sizeof(domes_vault_config_t));
    } else {
    ...
    domes_rwlock_init(&g_vault_ctx.global_lock);
    g_vault_ctx.initialized = true;

    return 0;
}

void domes_vault_cleanup(void) {
    if (!g_vault_ctx.initialized) {
        return;
    }

    domes_rwlock_destroy(&g_vault_ctx.global_lock);
    memset(&g_vault_ctx, 0, sizeof(g_vault_ctx));
}

int domes_vault_open(const char* vault_id, const char* password, domes_vault_t** vault) {
    if (!vault_id || !vault) {
        return -1;
    }

    if (!g_vault_ctx.initialized) {
        domes_vault_init(NULL);
    }

    domes_vault_t* v = (domes_vault_t*)calloc(1, sizeof(domes_vault_t));
    if (!v) {
        return -1;
    }
    ...
    domes_rwlock_init(&v->lock);
    memcpy(&v->manager, &g_vault_ctx.default_config, sizeof(domes_vault_config_t));
...
void domes_vault_close(domes_vault_t* vault) {
    if (!vault) {
        return;
    }

    domes_rwlock_wrlock(&vault->lock);
    ...
    if (vault->entries) {
        for (size_t i = 0; i < vault->entry_count; i++) {
            ...
            domes_vault_free_metadata(&entry->metadata);
        }
        free(vault->entries);
    }
    ...
    domes_rwlock_unlock(&vault->lock);
    domes_rwlock_destroy(&vault->lock);

    free(vault);
}

int domes_vault_lock(domes_vault_t* vault) {
    if (!vault) {
        return -1;
    }
...
int domes_vault_unlock(domes_vault_t* vault, const char* password) {
    if (!vault || !password) {
        return -1;
    }
...
bool domes_vault_is_locked(domes_vault_t* vault) {
    if (!vault) {
        return true;
    }

    domes_rwlock_rdlock(&vault->lock);
...
static credential_entry_t* find_entry(domes_vault_t* vault, const char* cred_id) {
    for (size_t i = 0; i < vault->entry_count; i++) {
...
int domes_vault_store(domes_vault_t* vault,
                      const char* cred_id,
                      domes_vault_cred_type_t type,
                      const uint8_t* data, size_t data_len,
                      const domes_vault_acl_t* acl) {
    ...
    if (acl) {
        ...
        entry->acl.entries = (domes_vault_acl_entry_t*)malloc(
            acl->count * sizeof(domes_vault_acl_entry_t));
...
int domes_vault_retrieve(domes_vault_t* vault,
                         const char* cred_id,
                         const char* agent_id,
    ...
    if (!domes_vault_check_access(vault, cred_id, agent_id, DOMES_VAULT_OP_READ)) {
        domes_rwlock_unlock(&vault->lock);
        return -4;
    }
...
int domes_vault_delete(domes_vault_t* vault,
                       const char* cred_id,
                       const char* agent_id) {
    if (!vault || !cred_id) {
        return -1;
...
bool domes_vault_exists(domes_vault_t* vault, const char* cred_id) {
    if (!vault || !cred_id) {
        return false;
    }
...
int domes_vault_update(domes_vault_t* vault,
                       const char* cred_id,
                       const uint8_t* data, size_t data_len,
                       const char* agent_id) {
    ...
    if (!domes_vault_check_access(vault, cred_id, agent_id, DOMES_VAULT_OP_WRITE)) {
        return -2;
    }
...
bool domes_vault_check_access(domes_vault_t* vault,
                               const char* cred_id,
                               const char* agent_id,
                               domes_vault_operation_t operation) {
    ...
    for (size_t i = 0; i < entry->acl.count; i++) {
        domes_vault_acl_entry_t* acl = &entry->acl.entries[i];
...
int domes_vault_grant_access(domes_vault_t* vault,
                              const char* cred_id,
                              const char* agent_id,
                              uint32_t operations,
                              uint64_t expires_at) {
    ...
    domes_vault_acl_entry_t* new_entries = realloc(entry->acl.entries,
                                                    new_count * sizeof(domes_vault_acl_entry_t));
    if (!new_entries) {
        domes_rwlock_unlock(&vault->lock);
        return -3;
    }
...
int domes_vault_revoke_access(domes_vault_t* vault,
                               const char* cred_id,
                               const char* agent_id) {
    if (!vault || !cred_id || !agent_id) {
        return -1;
    ...
    for (size_t i = 0; i < entry->acl.count; i++) {
        if (strcmp(entry->acl.entries[i].agent_id, agent_id) == 0) {
            ...
            memmove(&entry->acl.entries[i], &entry->acl.entries[i + 1],
                    (entry->acl.count - i - 1) * sizeof(domes_vault_acl_entry_t));
            entry->acl.count--;
            domes_rwlock_unlock(&vault->lock);
            return 0;
        }
...
int domes_vault_get_metadata(domes_vault_t* vault,
                              const char* cred_id,
                              domes_vault_metadata_t* metadata) {
...
void domes_vault_free_metadata(domes_vault_metadata_t* metadata) {
    if (!metadata) {
        return;
    }
    ...
    memset(metadata, 0, sizeof(domes_vault_metadata_t));
}

int domes_vault_list(domes_vault_t* vault,
                     domes_vault_cred_type_t type,
                     domes_vault_metadata_t** metadata_array,
                     size_t* count) {
    ...
    domes_vault_metadata_t* arr = calloc(match_count, sizeof(domes_vault_metadata_t));
    if (!arr) {
        domes_rwlock_unlock(&vault->lock);
        return -2;
    }
...
void domes_vault_free_list(domes_vault_metadata_t* metadata_array, size_t count) {
    if (!metadata_array) {
        return;
    }

    for (size_t i = 0; i < count; i++) {
        domes_vault_free_metadata(&metadata_array[i]);
    }
    free(metadata_array);
}

int domes_vault_get_acl(domes_vault_t* vault,
                        const char* cred_id,
                        domes_vault_acl_t* acl) {
    ...
    acl->entries = calloc(entry->acl.count, sizeof(domes_vault_acl_entry_t));
    for (size_t i = 0; i < entry->acl.count; i++) {
...
void domes_vault_free_acl(domes_vault_acl_t* acl) {
    if (!acl || !acl->entries) {
        return;
    }
...
const char* domes_vault_cred_type_string(domes_vault_cred_type_t type) {
    switch (type) {
        case DOMES_VAULT_CRED_PASSWORD:    return "password";
        case DOMES_VAULT_CRED_TOKEN:       return "token";
        case DOMES_VAULT_CRED_KEY:         return "key";
        case DOMES_VAULT_CRED_CERTIFICATE: return "certificate";
        case DOMES_VAULT_CRED_SECRET:      return "secret";
        case DOMES_VAULT_CRED_NOTE:        return "note";
        default:                           return "unknown";
    }
}

const char* domes_vault_operation_string(domes_vault_operation_t op) {
    switch (op) {
        case DOMES_VAULT_OP_READ:   return "read";
        case DOMES_VAULT_OP_WRITE:  return "write";
        case DOMES_VAULT_OP_DELETE: return "delete";
        case DOMES_VAULT_OP_EXPORT: return "export";
        default:                    return "unknown";
    }
}

int domes_vault_generate_password(char* password_out, size_t length) {
...
int domes_vault_generate_keypair(char* public_key_out, size_t* pub_len,
...
int domes_vault_export(domes_vault_t* vault,
                        const char* export_path,
                        const char* password,
                        const char* agent_id) {
    DOMES_UNUSED(vault);
    DOMES_UNUSED(export_path);
    DOMES_UNUSED(password);
    DOMES_UNUSED(agent_id);
    return -1;
}

int domes_vault_import(domes_vault_t* vault,
                        const char* import_path,
                        const char* password,
                        const char* agent_id) {
    DOMES_UNUSED(vault);
    DOMES_UNUSED(import_path);
    DOMES_UNUSED(password);
    DOMES_UNUSED(agent_id);
    return -1;
}
