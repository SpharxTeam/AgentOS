...
static struct {
    int initialized;
    domes_net_security_config_t manager;
...
int domes_net_security_init(const domes_net_security_config_t* manager) {
    ...
    if (manager) {
        ...
        g_net_security.manager.tls.min_version = DOMES_TLS_1_2;
        g_net_security.manager.tls.max_version = DOMES_TLS_1_3;
...
int domes_net_check_access(const char* host, uint16_t port, domes_protocol_t protocol, const char* direction) {
    ...
    if (g_net_security.manager.http.enforce_https && protocol == DOMES_PROTO_HTTP) {
