 ...
 * @brief 统一日志和配置模块构建兼容性测�? * @copyright (c) 2026 SPHARX. All Rights Reserved.
 *
 * 此测试程序验证统一日志和配置模块的头文件包含和基本API兼容性�? * 不执行实际功能测试，仅验证编译时兼容性�? */
...
int main(void) {
    ...
    // 日志初始化函数（声明存在�?    printf("   日志初始化函数：log_init()\n");
    ...
    // 配置值类�?    config_value_t* config_val = NULL;
    printf("   配置值类型大小：%zu字节\n", sizeof(config_val));
    
    // 配置源类�?    config_source_type_t source_type = CONFIG_SOURCE_FILE;
    printf("   配置源类型：%d\n", source_type);
    
    // 配置验证器类�?    validator_type_t validator_type = VALIDATOR_TYPE_RANGE;
    printf("   验证器类型：%d\n", validator_type);
    
    /* 向后兼容层验�?*/
    printf("\n3. 验证向后兼容层：\n");
    
    // 日志兼容�?    printf("   日志兼容层：logging_compat.h\n");
    
    // 配置兼容�?    printf("   配置兼容层：config_compat.h\n");
    
    /* 简单功能测�?*/
    printf("\n4. 简单功能测试：\n");
    
    // 测试日志级别字符�?    const char* level_str = log_level_to_string(LOG_LEVEL_INFO);
    if (level_str) {
        printf("   日志级别字符串：%s\n", level_str);
    }
