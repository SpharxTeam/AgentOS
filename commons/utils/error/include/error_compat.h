 ...
 *
 * 本文件提供从其他错误处理模块到统一错误处理模块（commons/utils/error/）的
 * 向后兼容映射，支持渐进式迁移。
 *
 * 设计原则：
...
/* ==================== atoms/utils/error/ 模块兼容层 ==================== */
/**
 * 为 atoms/utils/error/include/error.h 提供的兼容层。
...
/* 将 atoms/utils/error/ 的错误码宏名映射到兼容层 */
#ifdef AGENTOS_USE_COMPAT_ERRORS
    /* 定义原 atoms/utils/error/ 模块的宏名，映射到兼容层 */
