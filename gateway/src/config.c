...
static agentos_error_t parse_server_config(cJSON* json, server_config_t* manager) {
    ...
    if (health_check_interval && cJSON_IsNumber(health_check_interval)) {
        manager->health_check_interval = (uint32_t)health_check_interval->valueint;
    }
 ...
 * @return AGENTOS_SUCCESS 成功
 */
static agentos_error_t parse_log_config(cJSON* json, log_config_t* manager) {
    if (!json || !manager) return AGENTOS_EINVAL;
    
    PARSE_STRING_FIELD(json, "log_file", manager->log_file);
    PARSE_NUMBER_FIELD(json, "log_level", manager->log_level, int);
    PARSE_NUMBER_FIELD(json, "log_rotation", manager->log_rotation, int);
    PARSE_NUMBER_FIELD(json, "log_backup_count", manager->log_backup_count, int);
 ...
 * @return AGENTOS_SUCCESS 成功
 */
static agentos_error_t parse_security_config(cJSON* json, security_config_t* manager) {
    if (!json || !manager) return AGENTOS_EINVAL;
    
    PARSE_STRING_FIELD(json, "jwt_secret", manager->jwt_secret);
    PARSE_NUMBER_FIELD(json, "jwt_expire", manager->jwt_expire, uint32_t);
    PARSE_NUMBER_FIELD(json, "max_login_attempts", manager->max_login_attempts, uint32_t);
    PARSE_NUMBER_FIELD(json, "lockout_duration", manager->lockout_duration, uint32_t);
    PARSE_STRING_FIELD(json, "allowed_origins", manager->allowed_origins);
    PARSE_BOOL_FIELD(json, "enable_auth", manager->enable_auth);
 ...
 * @return AGENTOS_SUCCESS 成功
 */
static agentos_error_t parse_performance_config(cJSON* json, performance_config_t* manager) {
    if (!json || !manager) return AGENTOS_EINVAL;
    
    cJSON* thread_pool_size = cJSON_GetObjectItem(json, "thread_pool_size");
    if (thread_pool_size && cJSON_IsNumber(thread_pool_size)) {
        manager->thread_pool_size = (size_t)thread_pool_size->valueint;
    }
    ...
    if (connection_pool_size && cJSON_IsNumber(connection_pool_size)) {
        manager->connection_pool_size = (size_t)connection_pool_size->valueint;
    }
    ...
    if (request_queue_size && cJSON_IsNumber(request_queue_size)) {
        manager->request_queue_size = (uint32_t)request_queue_size->valueint;
    }
    ...
    if (response_buffer_size && cJSON_IsNumber(response_buffer_size)) {
        manager->response_buffer_size = (uint32_t)response_buffer_size->valueint;
    }
    ...
    if (enable_compression && cJSON_IsBool(enable_compression)) {
        manager->enable_compression = cJSON_IsTrue(enable_compression) ? 1 : 0;
    }
...
static void apply_env_overrides(gateway_config_t* manager) {
    ...
    if (env_log_level) {
        if (strcmp(env_log_level, "DEBUG") == 0) manager->log.log_level = LOG_LEVEL_DEBUG;
        else if (strcmp(env_log_level, "INFO") == 0) manager->log.log_level = LOG_LEVEL_INFO;
        else if (strcmp(env_log_level, "WARN") == 0) manager->log.log_level = LOG_LEVEL_WARN;
        else if (strcmp(env_log_level, "ERROR") == 0) manager->log.log_level = LOG_LEVEL_ERROR;
        AGENTOS_LOG_INFO("Override log level from env: %s", env_log_level);
    }
    
    const char* env_log_path = getenv("AGENTOS_LOG_PATH");
    if (env_log_path) {
        free(manager->log.log_file);
        manager->log.log_file = strdup(env_log_path);
    ...
    if (env_thread_pool) {
        manager->performance.thread_pool_size = (uint32_t)atoi(env_thread_pool);
    ...
    if (env_conn_pool) {
        manager->performance.connection_pool_size = (uint32_t)atoi(env_conn_pool);
 ...
 * @return AGENTOS_SUCCESS 成功
 */
static agentos_error_t parse_config_file(const char* config_path, gateway_config_t* manager) {
    if (!config_path || !manager) return AGENTOS_EINVAL;
    
    /* 读取配置文件 */
    FILE* file = fopen(config_path, "r");
    if (!file) {
        AGENTOS_LOG_ERROR("Failed to open manager file: %s", config_path);
        return AGENTOS_EIO;
    }
    ...
    if (!json) {
        AGENTOS_LOG_ERROR("Failed to parse manager JSON");
    ...
    if (server_json) {
        parse_server_config(server_json, &manager->server);
    }
    
    cJSON* log_json = cJSON_GetObjectItem(json, "log");
    if (log_json) {
        parse_log_config(log_json, &manager->log);
    }
    
    cJSON* security_json = cJSON_GetObjectItem(json, "security");
    if (security_json) {
        parse_security_config(security_json, &manager->security);
    }
    
    cJSON* performance_json = cJSON_GetObjectItem(json, "performance");
    if (performance_json) {
        parse_performance_config(performance_json, &manager->performance);
    }
    
    cJSON_Delete(json);
    
    /* 应用环境变量覆盖 */
    apply_env_overrides(manager);
...
static void* config_watcher_thread(void* arg) {
    ...
    while (atomic_load(&mgr->running)) {
        ...
        if (mgr->config_path && file_was_modified(mgr->config_path, &last_mtime)) {
            AGENTOS_LOG_INFO("manager file changed, reloading...");
            
            /* 重新加载配置 */
            gateway_config_t new_config;
            memcpy(&new_config, &default_config, sizeof(gateway_config_t));
            ...
            if (err == AGENTOS_SUCCESS) {
                ...
                if (mgr->watcher.callback) {
                    mgr->watcher.callback("manager.reload", "true", mgr->watcher.user_data);
                }
                
                /* 更新配置 */
                memcpy(&mgr->manager, &new_config, sizeof(gateway_config_t));
...
config_manager_t* config_manager_create(const char* config_path) {
    ...
    /* 初始化配置 */
    memcpy(&mgr->manager, &default_config, sizeof(gateway_config_t));
    ...
    if (config_path) {
        ...
        /* 加载配置文件 */
        agentos_error_t err = parse_config_file(config_path, &mgr->manager);
        if (err != AGENTOS_SUCCESS) {
            AGENTOS_LOG_WARN("Failed to load manager file, using defaults");
        }
    }
    ...
    AGENTOS_LOG_INFO("manager manager created");
    return mgr;
}
...
void config_manager_destroy(config_manager_t* mgr) {
    ...
    free(mgr->manager.server.metrics_path);
    free(mgr->manager.log.log_file);
    free(mgr->manager.security.jwt_secret);
    free(mgr->manager.security.allowed_origins);
    free(mgr->config_path);
    
    free(mgr);
    
    AGENTOS_LOG_INFO("manager manager destroyed");
}
...
agentos_error_t config_manager_get_config(config_manager_t* mgr, gateway_config_t* out_config) {
    ...
    pthread_mutex_lock(&mgr->lock);
    memcpy(out_config, &mgr->manager, sizeof(gateway_config_t));
    pthread_mutex_unlock(&mgr->lock);
...
agentos_error_t config_manager_get_server_config(config_manager_t* mgr, server_config_t* out_config) {
    ...
    pthread_mutex_lock(&mgr->lock);
    memcpy(out_config, &mgr->manager.server, sizeof(server_config_t));
    pthread_mutex_unlock(&mgr->lock);
...
agentos_error_t config_manager_get_log_config(config_manager_t* mgr, log_config_t* out_config) {
    ...
    pthread_mutex_lock(&mgr->lock);
    memcpy(out_config, &mgr->manager.log, sizeof(log_config_t));
    pthread_mutex_unlock(&mgr->lock);
...
agentos_error_t config_manager_get_security_config(config_manager_t* mgr, security_config_t* out_config) {
    ...
    pthread_mutex_lock(&mgr->lock);
    memcpy(out_config, &mgr->manager.security, sizeof(security_config_t));
    pthread_mutex_unlock(&mgr->lock);
...
agentos_error_t config_manager_get_performance_config(config_manager_t* mgr, performance_config_t* out_config) {
    ...
    pthread_mutex_lock(&mgr->lock);
    memcpy(out_config, &mgr->manager.performance, sizeof(performance_config_t));
    pthread_mutex_unlock(&mgr->lock);
...
agentos_error_t config_manager_set_value(config_manager_t* mgr, const char* key, const char* value) {
    ...
    if (strcmp(key, "server.host") == 0) {
        free(mgr->manager.server.host);
        mgr->manager.server.host = strdup(value);
    } else if (strcmp(key, "server.http_port") == 0) {
        mgr->manager.server.http_port = (uint16_t)atoi(value);
    } else if (strcmp(key, "server.ws_port") == 0) {
        mgr->manager.server.ws_port = (uint16_t)atoi(value);
    } else if (strcmp(key, "log.log_level") == 0) {
        mgr->manager.log.log_level = atoi(value);
    } else if (strcmp(key, "security.enable_auth") == 0) {
        mgr->manager.security.enable_auth = atoi(value);
    } else {
        pthread_mutex_unlock(&mgr->lock);
...
agentos_error_t config_manager_export_json(config_manager_t* mgr, char** out_json) {
    ...
    cJSON_AddStringToObject(server_json, "host", mgr->manager.server.host);
    cJSON_AddNumberToObject(server_json, "http_port", mgr->manager.server.http_port);
    cJSON_AddNumberToObject(server_json, "ws_port", mgr->manager.server.ws_port);
    cJSON_AddNumberToObject(server_json, "metrics_port", mgr->manager.server.metrics_port);
    cJSON_AddStringToObject(server_json, "metrics_path", mgr->manager.server.metrics_path);
    cJSON_AddNumberToObject(server_json, "max_request_size", mgr->manager.server.max_request_size);
    cJSON_AddNumberToObject(server_json, "max_connections", mgr->manager.server.max_connections);
    cJSON_AddNumberToObject(server_json, "request_timeout", mgr->manager.server.request_timeout);
    cJSON_AddNumberToObject(server_json, "session_timeout", mgr->manager.server.session_timeout);
    cJSON_AddNumberToObject(server_json, "health_check_interval", mgr->manager.server.health_check_interval);
    cJSON_AddItemToObject(json, "server", server_json);
    
    /* 日志配置 */
    cJSON* log_json = cJSON_CreateObject();
    cJSON_AddStringToObject(log_json, "log_file", mgr->manager.log.log_file);
    cJSON_AddNumberToObject(log_json, "log_level", mgr->manager.log.log_level);
    cJSON_AddNumberToObject(log_json, "log_rotation", mgr->manager.log.log_rotation);
    cJSON_AddNumberToObject(log_json, "log_backup_count", mgr->manager.log.log_backup_count);
    cJSON_AddItemToObject(json, "log", log_json);
    
    /* 安全配置 */
    cJSON* security_json = cJSON_CreateObject();
    cJSON_AddStringToObject(security_json, "jwt_secret", mgr->manager.security.jwt_secret);
    cJSON_AddNumberToObject(security_json, "jwt_expire", mgr->manager.security.jwt_expire);
    cJSON_AddNumberToObject(security_json, "max_login_attempts", mgr->manager.security.max_login_attempts);
    cJSON_AddNumberToObject(security_json, "lockout_duration", mgr->manager.security.lockout_duration);
    cJSON_AddStringToObject(security_json, "allowed_origins", mgr->manager.security.allowed_origins);
    cJSON_AddNumberToObject(security_json, "enable_auth", mgr->manager.security.enable_auth);
    cJSON_AddItemToObject(json, "security", security_json);
    
    /* 性能配置 */
    cJSON* performance_json = cJSON_CreateObject();
    cJSON_AddNumberToObject(performance_json, "thread_pool_size", mgr->manager.performance.thread_pool_size);
    cJSON_AddNumberToObject(performance_json, "connection_pool_size", mgr->manager.performance.connection_pool_size);
    cJSON_AddNumberToObject(performance_json, "request_queue_size", mgr->manager.performance.request_queue_size);
    cJSON_AddNumberToObject(performance_json, "response_buffer_size", mgr->manager.performance.response_buffer_size);
    cJSON_AddNumberToObject(performance_json, "enable_compression", mgr->manager.performance.enable_compression);
...
agentos_error_t config_manager_validate(config_manager_t* mgr, char** out_error) {
    ...
    /* 验证安全配置 */
    if (mgr->manager.security.enable_auth && !mgr->manager.security.jwt_secret) {
