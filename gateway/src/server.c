...
/* 全局服务器实例（用于信号处理） */
static gateway_server_t* g_server_instance = NULL;
...
agentos_error_t server_validate_config(const gateway_config_t* cfg) {
...
agentos_error_t server_init_subsystems(gateway_server_t* server) {
    ...
    /* 创建健康检查器 */
    server->health = health_checker_create(server->manager.health_interval_sec);
    if (!server->health) {
...
void server_destroy_subsystems(gateway_server_t* server) {
    if (!server) return;
...
agentos_error_t server_start_gateways(gateway_server_t* server) {
    if (!server) return AGENTOS_EINVAL;
    ...
    server->gateways[GATEWAY_TYPE_HTTP] = http_gateway_create(
        server->manager.http_host,
        server->manager.http_port,
        server);
    if (!server->gateways[GATEWAY_TYPE_HTTP]) {
        AGENTOS_LOG_ERROR("Failed to create HTTP gateway on %s:%u",
            server->manager.http_host, server->manager.http_port);
    ...
    /* 创建并启动 Stdio 网关（可选） */
    if (server->manager.enable_stdio) {
...
void server_stop_gateways(gateway_server_t* server) {
    if (!server) return;
...
agentos_error_t gateway_server_create(
    ...
    /* 分配服务器实例 */
    gateway_server_t* server = (gateway_server_t*)calloc(1, sizeof(gateway_server_t));
    ...
    if (manager) {
        ...
        server_set_default_config(&server->manager);
    }
    
    /* 验证配置 */
    agentos_error_t err = server_validate_config(&server->manager);
    if (err != AGENTOS_SUCCESS) {
        free(server);
        return err;
    }
    ...
    *out_server = server;
    AGENTOS_LOG_INFO("gateway server instance created");
    return AGENTOS_SUCCESS;
}

agentos_error_t gateway_server_start(gateway_server_t* server) {
    if (!server) return AGENTOS_EINVAL;
    ...
    AGENTOS_LOG_INFO("gateway server started with %zu gateways", server->gateway_count);
    ...
    AGENTOS_LOG_INFO("gateway server stopped");
    return AGENTOS_SUCCESS;
}

void gateway_server_stop(gateway_server_t* server) {
    if (!server) return;
...
agentos_error_t gateway_server_wait(
...
void gateway_server_destroy(gateway_server_t* server) {
    if (!server) return;
    
    /* 确保服务器已停止 */
    int state = atomic_load(&server->state);
    if (state == SERVER_STATE_RUNNING) {
        gateway_server_stop(server);
        gateway_server_wait(server, 5000);
    }
    ...
    free(server);
    AGENTOS_LOG_INFO("gateway server instance destroyed");
}

agentos_error_t gateway_server_get_health(
    gateway_server_t* server,
    char** out_json) {
...
agentos_error_t gateway_server_get_metrics(
    gateway_server_t* server,
    char** out_metrics) {
...
agentos_error_t gateway_server_get_stats(
    gateway_server_t* server,
    char** out_stats) {
...
agentos_error_t gateway_run(const gateway_config_t* manager) {
    gateway_server_t* server = NULL;
    agentos_error_t err = gateway_server_create(manager, &server);
    if (err != AGENTOS_SUCCESS) {
        return err;
    }
...
const char* gateway_get_version(void) {
    return GATEWAY_API_VERSION_MAJOR "." GATEWAY_API_VERSION_MINOR "." GATEWAY_API_VERSION_PATCH;
}

uint32_t gateway_get_abi_version(void) {
    return GATEWAY_ABI_VERSION;
}
