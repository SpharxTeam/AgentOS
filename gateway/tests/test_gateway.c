...
TEST(health_register_check) {
    ...
    /* 等待检查执�?*/
    agentos_time_nanosleep(1000000000);  /* 1�?*/
...
TEST(telemetry_counter) {
    ...
    agentos_error_t err = telemetry_increment_counter(t, "gateway_requests_total", 1.0, "http,GET");
    ASSERT_EQ(err, AGENTOS_SUCCESS);
    ...
    ASSERT(strstr(metrics, "gateway_requests_total") != NULL);
...
TEST(telemetry_gauge) {
    ...
    agentos_error_t err = telemetry_set_gauge(t, "gateway_sessions_active", 42.0, NULL);
    ASSERT_EQ(err, AGENTOS_SUCCESS);
