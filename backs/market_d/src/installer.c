...
int installer_uninstall_agent(market_service_t* service, const char* agent_id) {
    ...
    // 生成安装路径
    char install_path[256];
    snprintf(install_path, sizeof(install_path), "%s/agents/%s", service->config.storage_path, agent_id);
...
int installer_uninstall_skill(market_service_t* service, const char* skill_id) {
    ...
    // 生成安装路径
    char install_path[256];
    snprintf(install_path, sizeof(install_path), "%s/skills/%s", service->config.storage_path, skill_id);
