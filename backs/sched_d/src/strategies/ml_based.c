 ...
 */
static int ml_based_create(const sched_config_t* config, void** data) {
    ml_based_data_t* mld = (ml_based_data_t*)malloc(sizeof(ml_based_data_t));
    if (!mld) {
        return -1;
    }

    mld->max_agents = config->max_agents;
    ...
    mld->agent_count = 0;
    mld->model_path = config->ml_model_path ? strdup(config->ml_model_path) : NULL;
    mld->model = NULL;
