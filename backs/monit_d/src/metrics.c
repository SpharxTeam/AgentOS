 ...
 * @brief 初始化指标系统
 */
int metrics_init(const metrics_config_t* config) {
    ...
    g_metrics.export_interval_ms = config ? config->export_interval_ms : DEFAULT_EXPORT_INTERVAL_MS;
    g_metrics.retention_seconds = config ? config->retention_seconds : DEFAULT_RETENTION_SECONDS;
