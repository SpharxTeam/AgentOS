...
```bash
# 方式 1: 直接启动
./build/backs/tool_d/agentos-tool-d

# 方式 2: 指定配置文件
./build/backs/tool_d/agentos-tool-d --config ../config/service/tool_d/tool.yaml
```
...
cat > my_tool.json << EOF
{
  ...
  "entry_point": "/opt/agentos/tools/weather.py"
}
EOF

curl -X POST http://localhost:8084/api/v1/tools/register \
...
```bash
# 同步执行
curl -X POST http://localhost:8084/api/v1/tools/weather_query/execute \
  -H "Content-Type: application/json" \
  -d '{"city": "北京", "date": "2026-03-25"}'

# 异步执行
curl -X POST http://localhost:8084/api/v1/tools/weather_query/execute?async=true \
  -H "Content-Type: application/json" \
  -d '{"city": "上海"}'
...
```bash
# 查看工具执行统计
curl http://localhost:8084/api/v1/tools/weather_query/stats
...
response = requests.post(
    'http://localhost:8084/api/v1/tools/register',
    json={
        'id': 'test-tool',
        'name': '测试工具',
...
# 测试工具执行
response = requests.post(
    'http://localhost:8084/api/v1/tools/test-tool/execute',
    json={'param': 'value'}
)
...
- [技能开发指南](../../partdocs/guides/folder/create_skill.md) - 如何开发工具技能
- [契约验证规范](../../partdocs/specifications/contract_validation.md) - 输入输出验证标准
