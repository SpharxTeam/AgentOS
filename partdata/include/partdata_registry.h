...
#ifndef AGENTOS_PARTDATA_REGISTRY_H
#define AGENTOS_PARTDATA_REGISTRY_H
 ...
 */
typedef enum {
    PARTDATA_REG_AGENTS,    /* Agent 注册表 */
    PARTDATA_REG_SKILLS,    /* 技能注册表 */
    PARTDATA_REG_SESSIONS,  /* 会话注册表 */
    PARTDATA_REG_MAX
} partdata_registry_type_t;

/**
 * @brief Agent 记录结构
 */
typedef struct partdata_agent_record {
    char id[128];
    char name[256];
    char type[64];
    char version[32];
    ...
    uint64_t created_at;
    uint64_t updated_at;
} partdata_agent_record_t;

/**
 * @brief 技能记录结构
 */
typedef struct partdata_skill_record {
    char id[128];
    char name[256];
    char version[32];
...
} partdata_skill_record_t;

/**
 * @brief 会话记录结构
 */
typedef struct partdata_session_record {
    char id[128];
    char user_id[128];
    uint64_t created_at;
    ...
    char status[32];
} partdata_session_record_t;

/**
 * @brief 注册表迭代器
 */
typedef struct partdata_registry_iter partdata_registry_iter_t;
 ...
 * @param iter 迭代器
 * @param record 输出记录
 * @return partdata_error_t 错误码，返回 PARTDATA_ERR_NOT_FOUND 表示遍历结束
 */
...
#endif /* AGENTOS_PARTDATA_REGISTRY_H */
