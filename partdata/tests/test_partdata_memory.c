...
static void test_memory_init_shutdown(void) {
    ...
    partdata_error_t err = partdata_memory_init();
...
static void test_memory_pool_crud(void) {
    ...
    partdata_error_t err = partdata_memory_record_pool(&pool);
    if (err == PARTDATA_SUCCESS) {
        partdata_memory_pool_t get_pool;
        memset(&get_pool, 0, sizeof(get_pool));

        err = partdata_memory_get_pool(pool.pool_id, &get_pool);
        ...
        err = partdata_memory_update_pool_usage(pool.pool_id, 768 * 1024, 64);
        assert(err == PARTDATA_SUCCESS);
    }
...
static void test_memory_allocation_crud(void) {
    ...
    partdata_memory_allocation_t allocation;
    ...
    if (err == PARTDATA_SUCCESS) {
        err = partdata_memory_record_allocation(&allocation);
        if (err == PARTDATA_SUCCESS) {
            partdata_memory_allocation_t get_alloc;
            memset(&get_alloc, 0, sizeof(get_alloc));

            err = partdata_memory_get_allocation(allocation.allocation_id, &get_alloc);
            assert(err == PARTDATA_SUCCESS);
            ...
            err = partdata_memory_free_allocation(allocation.allocation_id);
            assert(err == PARTDATA_SUCCESS);
        }
    }
...
static void test_memory_stats(void) {
    ...
    partdata_error_t err = partdata_memory_get_stats(&pool_count, &total_allocations, &total_size);
    assert(err == PARTDATA_SUCCESS);
...
static void test_memory_invalid_params(void) {
    ...
    partdata_error_t err = partdata_memory_record_pool(NULL);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    partdata_memory_pool_t invalid_pool;
    memset(&invalid_pool, 0, sizeof(invalid_pool));
    err = partdata_memory_record_pool(&invalid_pool);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    err = partdata_memory_record_allocation(NULL);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    partdata_memory_allocation_t invalid_alloc;
    memset(&invalid_alloc, 0, sizeof(invalid_alloc));
    err = partdata_memory_record_allocation(&invalid_alloc);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    err = partdata_memory_get_pool(NULL, NULL);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    err = partdata_memory_get_allocation(NULL, NULL);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    err = partdata_memory_update_pool_usage(NULL, 0, 0);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    err = partdata_memory_free_allocation(NULL);
    assert(err == PARTDATA_ERR_INVALID_PARAM);
...
static void test_memory_not_found(void) {
    ...
    partdata_error_t err = partdata_memory_get_pool("nonexistent_id", &pool);
    assert(err == PARTDATA_ERR_NOT_FOUND);

    err = partdata_memory_get_allocation("nonexistent_id", &pool);
    assert(err == PARTDATA_ERR_NOT_FOUND);

    err = partdata_memory_update_pool_usage("nonexistent_id", 0, 0);
    assert(err == PARTDATA_ERR_NOT_FOUND);

    err = partdata_memory_free_allocation("nonexistent_id");
    assert(err == PARTDATA_ERR_NOT_FOUND);
...
static void test_memory_update_usage(void) {
    ...
    partdata_error_t err = partdata_memory_record_pool(&pool);
    if (err == PARTDATA_SUCCESS) {
        err = partdata_memory_update_pool_usage(pool.pool_id, 1536 * 1024, 128);
        assert(err == PARTDATA_SUCCESS);
...
int main(void) {
    printf("=== AgentOS Partdata Memory Unit Tests ===\n\n");
