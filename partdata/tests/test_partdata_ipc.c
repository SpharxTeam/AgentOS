...
static void test_ipc_init_shutdown(void) {
    ...
    partdata_error_t err = partdata_ipc_init();
...
static void test_ipc_channel_crud(void) {
    ...
    partdata_error_t err = partdata_ipc_record_channel(&channel);
    if (err == PARTDATA_SUCCESS) {
        partdata_ipc_channel_t get_ch;
        memset(&get_ch, 0, sizeof(get_ch));

        err = partdata_ipc_get_channel(channel.channel_id, &get_ch);
        ...
        err = partdata_ipc_update_channel_activity(channel.channel_id);
        assert(err == PARTDATA_SUCCESS);
    }
...
static void test_ipc_buffer_crud(void) {
    ...
    if (err == PARTDATA_SUCCESS) {
        err = partdata_ipc_record_buffer(&buffer);
        if (err == PARTDATA_SUCCESS) {
            partdata_ipc_buffer_t get_buf;
            memset(&get_buf, 0, sizeof(get_buf));

            err = partdata_ipc_get_buffer(buffer.buffer_id, &get_buf);
            assert(err == PARTDATA_SUCCESS);
...
static void test_ipc_stats(void) {
    ...
    partdata_error_t err = partdata_ipc_get_stats(&channel_count, &buffer_count, &total_size);
    assert(err == PARTDATA_SUCCESS);
...
static void test_ipc_invalid_params(void) {
    ...
    partdata_error_t err = partdata_ipc_record_channel(NULL);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    partdata_ipc_channel_t invalid_ch;
    memset(&invalid_ch, 0, sizeof(invalid_ch));
    err = partdata_ipc_record_channel(&invalid_ch);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    err = partdata_ipc_record_buffer(NULL);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    partdata_ipc_buffer_t invalid_buf;
    memset(&invalid_buf, 0, sizeof(invalid_buf));
    err = partdata_ipc_record_buffer(&invalid_buf);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    err = partdata_ipc_get_channel(NULL, NULL);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    err = partdata_ipc_get_buffer(NULL, NULL);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    err = partdata_ipc_update_channel_activity(NULL);
    assert(err == PARTDATA_ERR_INVALID_PARAM);
...
static void test_ipc_not_found(void) {
    ...
    partdata_error_t err = partdata_ipc_get_channel("nonexistent_id", &channel);
    assert(err == PARTDATA_ERR_NOT_FOUND);

    err = partdata_ipc_get_buffer("nonexistent_id", &channel);
    assert(err == PARTDATA_ERR_NOT_FOUND);

    err = partdata_ipc_update_channel_activity("nonexistent_id");
    assert(err == PARTDATA_ERR_NOT_FOUND);
...
static void test_ipc_multiple_channels(void) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        partdata_error_t err = partdata_ipc_record_channel(&channel);
        assert(err == PARTDATA_SUCCESS || err == PARTDATA_ERR_OUT_OF_MEMORY);
    }

    uint32_t count = 0;
    partdata_ipc_get_stats(&count, NULL, NULL);
...
int main(void) {
    printf("=== AgentOS Partdata IPC Unit Tests ===\n\n");
