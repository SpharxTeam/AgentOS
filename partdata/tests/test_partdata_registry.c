...
static void test_registry_init_shutdown(void) {
    ...
    partdata_error_t err = partdata_registry_init();
...
static void test_registry_agent_crud(void) {
    ...
    partdata_agent_record_t record;
    ...
    err = partdata_registry_add_agent(&record);
    if (err == PARTDATA_SUCCESS) {
        partdata_agent_record_t get_record;
        memset(&get_record, 0, sizeof(get_record));

        err = partdata_registry_get_agent(record.id, &get_record);
        assert(err == PARTDATA_SUCCESS);
        ...
        err = partdata_registry_update_agent(&record);
        assert(err == PARTDATA_SUCCESS);

        err = partdata_registry_delete_agent(record.id);
        assert(err == PARTDATA_SUCCESS);
...
static void test_registry_skill_crud(void) {
    ...
    partdata_skill_record_t record;
    memset(&record, 0, sizeof(record));
    ...
    err = partdata_registry_add_skill(&record);
    if (err == PARTDATA_SUCCESS) {
        partdata_skill_record_t get_record;
        memset(&get_record, 0, sizeof(get_record));

        err = partdata_registry_get_skill(record.id, &get_record);
        assert(err == PARTDATA_SUCCESS);
        assert(strcmp(get_record.name, record.name) == 0);

        err = partdata_registry_delete_skill(record.id);
        assert(err == PARTDATA_SUCCESS);
    }
...
static void test_registry_session_crud(void) {
    ...
    partdata_session_record_t record;
    memset(&record, 0, sizeof(record));
    ...
    err = partdata_registry_add_session(&record);
    if (err == PARTDATA_SUCCESS) {
        partdata_session_record_t get_record;
        memset(&get_record, 0, sizeof(get_record));

        err = partdata_registry_get_session(record.id, &get_record);
        assert(err == PARTDATA_SUCCESS);
        assert(strcmp(get_record.user_id, record.user_id) == 0);

        record.last_active_at = (uint64_t)time(NULL);
        err = partdata_registry_update_session(&record);
        assert(err == PARTDATA_SUCCESS);

        err = partdata_registry_delete_session(record.id);
        assert(err == PARTDATA_SUCCESS);
    }
...
static void test_registry_invalid_params(void) {
    ...
    partdata_agent_record_t agent_record;
    memset(&agent_record, 0, sizeof(agent_record));

    err = partdata_registry_add_agent(NULL);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    err = partdata_registry_add_agent(&agent_record);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    partdata_agent_record_t get_record;
    err = partdata_registry_get_agent(NULL, &get_record);
    assert(err == PARTDATA_ERR_INVALID_PARAM);

    err = partdata_registry_get_agent("nonexistent", &get_record);
    assert(err != PARTDATA_SUCCESS);
...
int main(void) {
    printf("=== AgentOS Partdata Registry Unit Tests ===\n\n");
