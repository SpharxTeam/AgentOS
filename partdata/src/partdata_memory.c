...
partdata_error_t partdata_memory_init(void) {
    ...
    ensure_directory("partdata/kernel/memory/pools");
    ensure_directory("partdata/kernel/memory/stats");
    ensure_directory("partdata/kernel/memory/allocations");
...
partdata_error_t partdata_memory_record_pool(const partdata_memory_pool_t* pool) {
    ...
    memcpy(&node->pool, pool, sizeof(partdata_memory_pool_t));
...
partdata_error_t partdata_memory_get_pool(const char* pool_id, partdata_memory_pool_t* pool) {
    if (!pool_id || !pool) {
        return PARTDATA_ERR_INVALID_PARAM;
    }
    ...
    while (node) {
        if (strcmp(node->pool.pool_id, pool_id) == 0) {
            memcpy(pool, &node->pool, sizeof(partdata_memory_pool_t));
            pthread_mutex_unlock(&g_memory_data.lock);
            return PARTDATA_SUCCESS;
        }
        node = node->next;
...
partdata_error_t partdata_memory_update_pool_usage(const char* pool_id, size_t used_size, uint32_t free_block_count) {
    if (!pool_id) {
        return PARTDATA_ERR_INVALID_PARAM;
    }
...
partdata_error_t partdata_memory_record_allocation(const partdata_memory_allocation_t* allocation) {
    ...
    memcpy(&node->allocation, allocation, sizeof(partdata_memory_allocation_t));
...
partdata_error_t partdata_memory_get_allocation(const char* allocation_id, partdata_memory_allocation_t* allocation) {
    ...
    while (node) {
        if (strcmp(node->allocation.allocation_id, allocation_id) == 0) {
            memcpy(allocation, &node->allocation, sizeof(partdata_memory_allocation_t));
            pthread_mutex_unlock(&g_memory_data.lock);
            return PARTDATA_SUCCESS;
        }
        node = node->next;
...
partdata_error_t partdata_memory_free_allocation(const char* allocation_id) {
    if (!allocation_id) {
        return PARTDATA_ERR_INVALID_PARAM;
    }
...
partdata_error_t partdata_memory_get_stats(uint32_t* pool_count, uint32_t* total_allocations, uint64_t* total_size) {
    if (!g_memory_data.initialized) {
        return PARTDATA_ERR_NOT_INITIALIZED;
    }
