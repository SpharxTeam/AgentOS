...
partdata_error_t partdata_ipc_init(void) {
    ...
    ensure_directory("partdata/kernel/ipc/channels");
    ensure_directory("partdata/kernel/ipc/buffers");
...
partdata_error_t partdata_ipc_record_channel(const partdata_ipc_channel_t* channel) {
    ...
    memcpy(&node->channel, channel, sizeof(partdata_ipc_channel_t));
...
partdata_error_t partdata_ipc_get_channel(const char* channel_id, partdata_ipc_channel_t* channel) {
    ...
    while (node) {
        if (strcmp(node->channel.channel_id, channel_id) == 0) {
            memcpy(channel, &node->channel, sizeof(partdata_ipc_channel_t));
            pthread_mutex_unlock(&g_ipc_data.lock);
            return PARTDATA_SUCCESS;
        }
        node = node->next;
...
partdata_error_t partdata_ipc_update_channel_activity(const char* channel_id) {
    if (!channel_id) {
        return PARTDATA_ERR_INVALID_PARAM;
    }
...
partdata_error_t partdata_ipc_record_buffer(const partdata_ipc_buffer_t* buffer) {
    ...
    memcpy(&node->buffer, buffer, sizeof(partdata_ipc_buffer_t));
...
partdata_error_t partdata_ipc_get_buffer(const char* buffer_id, partdata_ipc_buffer_t* buffer) {
    if (!buffer_id || !buffer) {
        return PARTDATA_ERR_INVALID_PARAM;
    }
    ...
    while (node) {
        if (strcmp(node->buffer.buffer_id, buffer_id) == 0) {
            memcpy(buffer, &node->buffer, sizeof(partdata_ipc_buffer_t));
            pthread_mutex_unlock(&g_ipc_data.lock);
            return PARTDATA_SUCCESS;
        }
        node = node->next;
...
partdata_error_t partdata_ipc_get_stats(uint32_t* channel_count, uint32_t* buffer_count, uint64_t* total_size) {
    if (!g_ipc_data.initialized) {
        return PARTDATA_ERR_NOT_INITIALIZED;
    }
