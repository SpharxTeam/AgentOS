...
```http
POST /api/v1/toolkit/{tool_id}/execute
Content-Type: application/json
...
# 方式 2: 指定配置文件
./build/daemon/tool_d/agentos-tool-d --manager ../manager/service/tool_d/tool.yaml
```
...
curl -X POST http://localhost:8084/api/v1/toolkit/register \
...
```bash
# 同步执行
curl -X POST http://localhost:8084/api/v1/toolkit/weather_query/execute \
  -H "Content-Type: application/json" \
  -d '{"city": "北京", "date": "2026-03-25"}'

# 异步执行
curl -X POST http://localhost:8084/api/v1/toolkit/weather_query/execute?async=true \
  -H "Content-Type: application/json" \
  -d '{"city": "上海"}'
...
```bash
# 查看工具执行统计
curl http://localhost:8084/api/v1/toolkit/weather_query/stats
...
response = requests.post(
    'http://localhost:8084/api/v1/toolkit/register',
    json={
        'id': 'test-tool',
        'name': '测试工具',
...
# 测试工具执行
response = requests.post(
    'http://localhost:8084/api/v1/toolkit/test-tool/execute',
    json={'param': 'value'}
)
