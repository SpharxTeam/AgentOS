...
int monitor_service_log(monitor_service_t* service, const log_info_t* log) {
    ...
    // 写入日志文件
    if (service->manager.log_file_path) {
        FILE* log_file = fopen(service->manager.log_file_path, "a");
        if (log_file) {
            fprintf(log_file, "%s\n", log_message);
            fclose(log_file);
        }
...
int monitor_service_generate_report(monitor_service_t* service, char** report) {
    ...
    written += snprintf(report_buffer + written, report_size - written, "2. 配置信息\n" 
                      ...
                      "   - 启用追踪: %s\n" 
                      "   - 启用告警: %s\n\n",
                      service->manager.metrics_collection_interval_ms,
                      service->manager.health_check_interval_ms,
                      service->manager.log_flush_interval_ms,
                      service->manager.alert_check_interval_ms,
                      service->manager.log_file_path ? service->manager.log_file_path : "未设置",
                      service->manager.metrics_storage_path ? service->manager.metrics_storage_path : "未设置",
                      service->manager.enable_tracing ? "是" : "否",
                      service->manager.enable_alerting ? "是" : "否");
