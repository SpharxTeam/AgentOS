...
在 AgentOS 项目架构中，Manager 模块承担以下关键角色：
...
Manager 模块严格遵循 AgentOS 的 **五维正交设计原则体系**，将架构思想转化为工程实践：

#### 系统观（System View）的体现
- **反馈闭环原则**：配置变更→验证→审计→回滚的完整闭环
- **层次分解原则**：基础配置→环境覆盖→运行时变量的三层架构
- **总体设计部原则**：集中式配置管理，统一协调各子系统行为

#### 内核观（Kernel View）的体现  
- **接口契约化原则**：所有配置通过 JSON Schema 明确定义接口契约
- **可插拔策略原则**：调度、安全、验证等策略均可配置化替换

#### 工程观（Engineering View）的体现
- **安全内生原则**：加密、权限、审计内置于配置系统核心
- **可观测性原则**：完整的配置变更审计、追踪和监控能力
- **资源确定性原则**：明确的配置生命周期管理（加载→验证→应用→清理）
- **跨平台一致性原则**：配置在不同环境（开发/预发/生产）保持行为一致

#### 设计美学（Aesthetic View）的体现
- **简约至上原则**：声明式配置，最小化接口复杂度
- **极致细节原则**：精确的 Schema 验证、友好的错误提示
- **人文关怀原则**：渐进式文档、完整的示例、便捷的调试工具
- **完美主义原则**：100% Schema 覆盖、零警告验证、完整的变更历史
...
python scripts/dev/validate_config.py \
  --file manager/kernel/settings.yaml
...
### 部署配置 (deployment/cupolas/environments.yaml)
...
# 部署到开发环境
python scripts/deploy/deploy_config.py \
  --source manager/deployment/cupolas/ \
  --env development
...
validation:
  ...
  schema_validation:
    enabled: true
    schema_dir: "${AGENTOS_ROOT}/manager/schema"
...
┌─────────────────────────────────────┐
│     环境覆盖配置 (中等优先级)        │
│  manager/environment/production.yaml │
└─────────────────────────────────────┘
              ↑ 覆盖
┌─────────────────────────────────────┐
│     基础配置 (最低优先级)            │
...
python scripts/dev/validate_config.py \
  --file manager/kernel/settings.yaml \
  --schema manager/schema/kernel-settings.schema.json
...
3. 检查文件权限
   ```bash
   ls -l manager/kernel/settings.yaml
   chmod 644 manager/kernel/settings.yaml
   ```
...
1. 检查安全策略
   ```bash
   cat manager/security/policy.yaml | grep -A 10 "default_policy"
   ```

2. 确认 Agent 权限
   ```bash
   cat manager/security/permission_rules.yaml
   ```
...
1. 检查文件路径
   ```bash
   ls -la manager/kernel/settings.yaml
   ```

2. 验证 YAML 语法
   ```bash
   python -c "import yaml; print(yaml.safe_load(open('manager/kernel/settings.yaml')))"
   ```

3. 检查环境变量引用
   ```bash
   grep "\${" manager/kernel/settings.yaml
   ```
