...
本文档提供了 AgentOS 中所有错误码的统一参考，包括错误码的分类、定义、使用方法和最佳实践。错误码是系统中错误处理的重要组成部分，统一的错误码管理可以提高系统的可维护性和可扩展性。
...
| 版本 | 日期 | 变更说明 |
|------|------|----------|
| v1.0.0 | 2026-03-21 | 初始版本，包含通用错误、核心循环错误、认知层错误、执行层错误、记忆层错误、系统调用错误、安全域错误和动态模块错误 |
...
| 错误码 | 宏定义 | 描述 | 处理建议 |
|--------|--------|------|----------|
| 0x6001 | AGENTOS_ERROR_DOMES_CREATE_FAILED | 安全域创建失败 | 检查配置和资源 |
| 0x6002 | AGENTOS_ERROR_DOME_CREATE_FAILED | 单个安全域创建失败 | 检查域配置 |
| 0x6003 | AGENTOS_ERROR_DOME_ACCESS_DENIED | 域访问拒绝 | 检查访问权限 |
| 0x6004 | AGENTOS_ERROR_DOME_SECURITY_VIOLATION | 安全违反 | 检查安全策略和访问模式 |
| 0x6005 | AGENTOS_ERROR_DOME_ISOLATION_FAILED | 隔离失败 | 检查隔离机制和资源 |
| 0x6006 | AGENTOS_ERROR_DOME_MONITOR_FAILED | 监控失败 | 检查监控配置和状态 |
| 0x6007 | AGENTOS_ERROR_DOME_POLICY_VIOLATION | 策略违反 | 检查安全策略设置 |
| 0x6008 | AGENTOS_ERROR_DOME_RESOURCE_EXHAUSTED | 资源耗尽 | 检查资源使用情况 |
