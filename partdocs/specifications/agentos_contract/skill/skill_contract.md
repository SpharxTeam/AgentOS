...
- **模块化与可插拔**: 技能作为独立的执行单元，通过契约实现动态加载和热插拔，支持社区贡献和生态演进
- **微内核思想**: 技能外置于内核，通过系统调用与内核交互，保证内核纯净和服务隔离
- **安全内生**: 通过 `required_permissions` 和 `dependencies` 字段将安全和依赖管理内嵌于契约定义中
- **层次分解**: 技能按类型分层 (tool/code/api/file/browser/db/shell),每类技能有特定的执行环境和权限边界
