...
**归属**: AgentOS 开放生态核心规范  
**许可证**: GPL-3.0  
**理论基础**: 认知层理论（双系统认知模型）、设计原则（模块化与可插拔）  
...
- **认知层理论**: Agent 契约体现了"双系统认知模型",通过 `models` 字段明确区分 System 1(快系统) 和 System 2(慢系统) 的配置，支持认知层的策略选择和资源匹配
- **系统工程方法**: 契约采用层次分解方法，从顶层结构到能力细节逐层展开，符合"从定性到定量的综合集成"思想
- **微内核思想**: 契约设计遵循"最小信息原则",只包含系统决策所必需的信息，保证内核纯净和服务外置
- **安全内生**: 通过 `required_permissions` 字段将安全机制内嵌于契约定义中，支持虚拟工位和权限裁决
...
> **注意**: 模型名称应与 `config/models.yaml` 中定义的名称一致。
...
3. **模型存在性检查**: `models` 中的模型名称必须在 `config/models.yaml` 中存在
