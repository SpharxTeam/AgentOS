...
- **微内核思想**: 内核只提供最核心的功能 (任务管理、记忆管理、会话管理),其他功能外置为服务
- **最小权限原则**: 每个系统调用只拥有完成其功能所必需的最小权限
- **安全内生设计**: 系统调用内部内置参数校验、权限检查、审计日志等安全机制
- **可观测性设计**: 所有系统调用自动记录 TraceID、耗时、错误码等可观测数据
...
2. **服务开发者**: backs/守护进程通过系统调用访问内核功能
3. **内核开发者**: 实现和维护系统调用接口
...
| 微内核 (Microkernel) | 只提供核心功能的最小化内核 | [架构设计原则](../架构设计原则.md) |
...
const char* config = "{\"user\":\"alice\",\"ttl\":3600}";
if (agentos_sys_session_create(config, &session_id) == AGENTOS_SUCCESS) {
...
| 创建会话 | `session_create` | config | session_id |
...
| [架构设计原则](../架构设计原则.md) | 本规范是架构原则在系统调用层面的具体实现 |
