...
**定义**: AgentOS 的核心服务层，包含 LLM 服务、市场服务、监控服务等用户态守护进程。

**中文优先**: 后端服务层 (Backs)

**来源**: [架构设计原则.md - 1.2.3 节](./架构设计原则.md)  
**参见**: [Domes](#domes-安全隔离层)、[Atoms](#atoms-内核层)
...
**定义**: 运行在用户态的后台服务进程，以 `_d` 后缀命名，如 `llm_d`、`market_d`。

**中文优先**: 守护进程 (Daemon)

**来源**: [架构设计原则.md - 1.2.4 节](./架构设计原则.md)  
**参见**: [Backs](#backs-后端服务层)、[Service Isolation](#service-isolation-服务隔离)

#### Domes (安全隔离层)

**定义**: AgentOS 的安全隔离层，提供虚拟工位、权限裁决、输入净化等安全机制。
...
**参见**: [Domes](#domes-安全隔离层)、[Sandbox](#sandbox-沙箱)
