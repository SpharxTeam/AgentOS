...
```powershell
# 使用 vcpkg 安装依赖
vcpkg install cmake pkg-config
...
User=agentos
Group=agentos
ExecStart=/usr/local/bin/agentosd --config /etc/agentos/agentos.yaml
...
```powershell
# 注册为 Windows 服务
agentosd --install --config C:\AgentOS\config\agentos.yaml
...
COPY --from=builder /src/AgentOS/config /etc/agentos
EXPOSE 8080 8081
CMD ["agentosd", "--config", "/etc/agentos/agentos.yaml"]
```
...
# 检查配置安全
agentos-cli security audit --config /etc/agentos/agentos.yaml
