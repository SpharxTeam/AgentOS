...
v1.0 是 AgentOS 的首个生产就绪版本，引入了完整的微内核架构（corekern）、三层运行时（coreloopthree）、四层记忆系统（memoryrovol）和安全穹顶（domes）。
...
agentos_cognition_create(const agentos_config_t* config,
...
include_directories(
    ...
    "${CMAKE_CURRENT_SOURCE_DIR}/../atoms/memoryrovol/include"
...
| 日志输出 | 检查 `partdata/logs/` | 格式正确、trace_id 连通 |
