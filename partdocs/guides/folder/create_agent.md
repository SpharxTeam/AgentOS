...
# 安装为守护进程
install(TARGETS my_agent_d DESTINATION lib/agentos/backs)
```
 ...
 * @return 0 成功；负值 错误码
 */
int my_agent_init(my_agent_t* agent, const agentos_config_t* config);
...
int my_agent_init(my_agent_t* agent, const agentos_config_t* config)
{
    ...
    // 解析配置
    agent->data_dir = config_get_string(config, "data_dir", "/var/lib/agentos/my_agent");
    agent->max_concurrent = config_get_int(config, "max_concurrent", 8);
...
int main(int argc, char* argv[])
{
    ...
    int ret = agentos_config_load("config/my_agent.yaml", &config);
    if (ret != 0) {
    ...
    // 初始化 Agent
    my_agent_t agent = { 0 };
    ret = my_agent_init(&agent, &config);
    if (ret != 0) {
...
agentos-cli agent register \
    ...
    --binary /usr/local/lib/agentos/backs/my_agent_d \
...
{
    my_agent_t agent = { 0 };
    agentos_config_t config = { .data_dir = "/tmp/test_agent" };

    int ret = my_agent_init(&agent, &config);
    ASSERT_EQUAL(0, ret);
