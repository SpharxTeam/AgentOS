...
int main(void)
{
    ...
    agentos_error_t err = agentos_agent_create(&config, &agent);
...
| **Skill** | Agent 的能力单元，遵循 Skill 契约 | dynamic/ |
...
[Service]
Type=notify
ExecStart=/usr/local/bin/agentos-core --config /etc/agentos/agentos.yaml
Restart=on-failure
RestartSec=5
LimitNOFILE=65535
