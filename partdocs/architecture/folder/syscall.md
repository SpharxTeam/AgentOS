...
系统调用层（Syscall Layer）是 AgentOS 提供的统一内核接口，封装微内核核心功能，为上层应用和服务提供稳定、安全、高效的系统调用抽象。

### 1.1 设计理念

```
┌─────────────────────────────────────────┐
│      应用层 (openhub/app/backs)          │
│  • 智能体应用 • 服务守护进程             │
└───────────────↓─────────────────────────┘
         系统调用接口
┌─────────────────────────────────────────┐
...
char* session_id;
sys_session_create(config, &session_id);
```
